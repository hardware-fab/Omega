...
#else
  //static uintptr_t uncached_area_ptr = 0xa0100000; GM change: original ESP pointer
  #define UNCACHED_START_ADDR 0x80080000
  #if SOC_NMEM >= 2
    #define UNCACHED_START_ADDR_BANK1 (0x80000000 + MEM_SIZE_MAIN)
    #if SOC_NMEM >= 4
      #define UNCACHED_START_ADDR_BANK2 (0x80000000 + MEM_SIZE_MAIN + MEM_SIZE_ADD)
      #define UNCACHED_START_ADDR_BANK3 (0x80000000 + MEM_SIZE_MAIN + MEM_SIZE_ADD*2)
    #endif
  #endif

  static uintptr_t uncached_area_ptr;
  static uintptr_t uncached_area_ptr_bank1;
  static uintptr_t uncached_area_ptr_bank2;
  static uintptr_t uncached_area_ptr_bank3;
...
//Remove this function - is just for testing
unsigned long get_uncached_pointer()
{
  return (unsigned long) uncached_area_ptr;
}

//This function initializes the static variables that keep track of the used memory positions.
//It seems that after the restart of the system, they are not automatically reset.
void init_uncached_pointers()
{
  uncached_area_ptr = UNCACHED_START_ADDR;
  printf("\nFirst bank starts from address %lx\n", uncached_area_ptr);
  #if SOC_NMEM >= 2
    uncached_area_ptr_bank1 = UNCACHED_START_ADDR_BANK1;
    printf("Second bank starts from address %lx\n", uncached_area_ptr_bank1);
    #if SOC_NMEM >= 4
      uncached_area_ptr_bank2 = UNCACHED_START_ADDR_BANK2;
      printf("Third bank starts from address %lx\n", uncached_area_ptr_bank2);
      uncached_area_ptr_bank3 = UNCACHED_START_ADDR_BANK3;
      printf("Fourth bank starts from address %lx\n", uncached_area_ptr_bank3);
    #endif
  #endif  
}
...
//GM change: this is a custom function that specifies on which memory bank the malloc should be executed
void *aligned_malloc_bank(int size, int bankID) {
  void *mem = 0;
  switch(bankID)
  {
    case 0:
      mem = (void *) uncached_area_ptr;
      uncached_area_ptr += size + CACHELINE_SIZE + sizeof(void*);
    break;
    case 1:
      mem = (void *) uncached_area_ptr_bank1;
      uncached_area_ptr_bank1 += size + CACHELINE_SIZE + sizeof(void*);
    break;
    case 2:
      mem = (void *) uncached_area_ptr_bank2;
      uncached_area_ptr_bank2 += size + CACHELINE_SIZE + sizeof(void*);
    break;
    case 3:
      mem = (void *) uncached_area_ptr_bank3;
      uncached_area_ptr_bank3 += size + CACHELINE_SIZE + sizeof(void*);
    break;
    default:
      printf("Called aligned_malloc_bank for a memory bank different from 0, 1, 2, 3. Terminating the function.\n");
      return NULL;
  }
...
static void esp_done(const struct fdt_scan_node *node, void *extra)
{
  struct esp_device **espdevs = (struct esp_device **)extra;
  const char *name = (*espdevs)[0].name;
  //(*espdevs)[ndev].compat = 0;   //OVERRIDE MESSO DA ME. VEDIAMO COSA SUCCEDE.
