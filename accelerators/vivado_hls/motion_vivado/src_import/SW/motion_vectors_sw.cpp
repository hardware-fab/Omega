  ...
  if (motion_vector_count == 1)
      ...
      if (mv_format == MV_FIELD && !dmv)
	{
	  motion_vertical_field_select[1][s] =
	    motion_vertical_field_select[0][s] = Get_Bits_sw (1);
	}

      motion_vector_sw (PMV[0][s], dmvector, h_r_size, v_r_size, dmv, mvscale,
		     0);
  ...
  else
    {
      ...
      motion_vertical_field_select[1][s] = Get_Bits_sw (1);

      motion_vector_sw (PMV[1][s], dmvector, h_r_size, v_r_size, dmv, mvscale,
		     0);
    }
}
...
void motion_vector_sw (int *PMV, int *dmvector, int h_r_size, int v_r_size, int dmv, int mvscale, int full_pel_vector)
{
  ...
  motion_code = Get_motion_code_sw ();

  motion_residual = (h_r_size != 0
		     && motion_code != 0) ? Get_Bits_sw (h_r_size) : 0;

  decode_motion_vector (&PMV[0], h_r_size, motion_code, motion_residual,
			full_pel_vector);

  if (dmv)
    dmvector[0] = Get_dmvector_sw ();


  /* vertical component */
  motion_code = Get_motion_code_sw ();
  motion_residual = (v_r_size != 0
		     && motion_code != 0) ? Get_Bits_sw (v_r_size) : 0;
