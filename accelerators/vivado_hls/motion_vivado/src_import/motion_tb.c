...
|     inRdbfr, inmvfs, inPMV : input data                                   |
+--------------------------------------------------------------------------+
*/
...
int main()
{
	int motion_vertical_field_select_sw[2][2];
	int PMV_sw[2][2][2];
	int motion_vertical_field_select_hw[2][2];
	int PMV_hw[2][2][2];
    int main_result = 0;
    int i,j,k;

    motion_main_sw(inRdbfr, inmvfs, inPMV, motion_vertical_field_select_sw, PMV_sw);
    motion_main_hw(inRdbfr, inmvfs, inPMV, motion_vertical_field_select_hw, PMV_hw);

    for (i = 0; i < 2; i++)
        for (j = 0; j < 2; j++)
        {
            //main_result += (motion_vertical_field_select_sw[i][j] != outmvfs[i][j]);
            printf("sw_result = %d -- hw_result = %d\n", motion_vertical_field_select_sw[i][j], motion_vertical_field_select_hw[i][j]);
        	main_result += (motion_vertical_field_select_sw[i][j] != motion_vertical_field_select_hw[i][j]);
            for (k = 0; k < 2; k++)
            	//main_result += (PMV_sw[i][j][k] != outPMV[i][j][k]);
                printf("sw_result = %d -- hw_result = %d\n", PMV_sw[i][j][k], PMV_hw[i][j][k]);
            	main_result += (PMV_sw[i][j][k] != PMV_hw[i][j][k]);
                //printf("%d\n", main_result);
        }

    printf("####################################\n");
    printf("test bench result: %d\n", main_result);
    printf("####################################\n");
