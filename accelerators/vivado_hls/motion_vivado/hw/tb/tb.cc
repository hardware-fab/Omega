...
int main(int argc, char **argv) {
    ...
    /* <<--params-->> */
	 const unsigned motion_mvfs = 4;
	 const unsigned motion_rdbfr = 512;
	 const unsigned motion_n = 10;
	 const unsigned motion_pwm = 8;
    ...
    in_words_adj = round_up(motion_rdbfr+motion_pwm+motion_mvfs, VALUES_PER_WORD);
    out_words_adj = round_up(motion_pwm+motion_mvfs, VALUES_PER_WORD);
    in_size = in_words_adj * (motion_n);
    out_size = out_words_adj * (motion_n);
    ...
    // Prepare input data
    for(unsigned i = 0; i < motion_n; i++)
        for(unsigned j = 0; j < motion_rdbfr+motion_pwm+motion_mvfs; j++)
        {   
            if(j<motion_rdbfr)
                inbuff[i * in_words_adj + j] = (word_t) inRdbfr[4*j] | (word_t) inRdbfr[4*j + 1]<<8 | (word_t) inRdbfr[4*j + 2]<<16 | (word_t) inRdbfr[4*j + 3]<<24;
            else if (j<motion_rdbfr + motion_mvfs)
                inbuff[i * in_words_adj + j] = (word_t) inmvfs[(j-motion_rdbfr)/2][(j-motion_rdbfr)%2];
            else
                inbuff[i * in_words_adj + j] = (word_t) inPMV[(j-motion_rdbfr-motion_mvfs)/(2*2)][((j-motion_rdbfr-motion_mvfs)%(2*2))/2][((j-motion_rdbfr-motion_mvfs)%(2))];
    ...
    // Set golden output
    for (unsigned i = 0; i < motion_n; i++)
        motion_main_sw((int *)&inbuff[i*in_words_adj], (int *)&outbuff_gold[i*out_words_adj]);
    ...
    int errors = 0;
    for(unsigned i = 0; i < motion_n; i++)
        for(unsigned j = 0; j < motion_pwm+motion_mvfs; j++)
	{   
