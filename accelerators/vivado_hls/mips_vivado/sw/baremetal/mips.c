...
#define DEV_NAME "sld,mips_vivado"
...
static void init_buf (token_t *in, token_t * gold)
{
	...
	for (i = 0; i < mips_n; i++)
		for (j = 0; j < mips_in; j++)
			in[i * in_words_adj + j] = (token_t) A[j];

	//Time measurement for software execution
	start_time = custom_gettime_nano();

	for (i = 0; i < mips_n; i++)
		mips_main_sw((int *)&in[i*in_words_adj], (int *)&gold[i*out_words_adj]);
...
int main(int argc, char * argv[])
{
	...
	if (DMA_WORD_PER_BEAT(sizeof(token_t)) == 0) {
		...
		in_words_adj = round_up(mips_in, DMA_WORD_PER_BEAT(sizeof(token_t)));
		out_words_adj = round_up(mips_out, DMA_WORD_PER_BEAT(sizeof(token_t)));
	}
	...
	ndev = probe(&espdevs, VENDOR_SLD, SLD_MIPS, DEV_NAME);
	if (ndev == 0) {
	...
	for (n = 0; n < ndev; n++) {
		...
		{
			print_time_us(total_time_hw[cnt]/(mips_n));
			printf("\n");
		}
		aligned_free(ptable);
		aligned_free(mem);
