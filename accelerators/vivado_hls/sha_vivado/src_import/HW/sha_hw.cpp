...
INT32 sha_info_count_lo_hw, sha_info_count_hi_hw;	/* 64-bit bit count */
INT32 sha_info_data_hw[16];
...
sha_init (INT32 sha_info_digest[5])
{
  ...
  sha_info_count_lo_hw = 0L;
  sha_info_count_hi_hw = 0L;
}
...
{
  if ((sha_info_count_lo_hw + ((INT32) count << 3)) < sha_info_count_lo_hw)
    {
      ++sha_info_count_hi_hw;
    }
  sha_info_count_lo_hw += (INT32) count << 3;
  sha_info_count_hi_hw += (INT32) count >> 29;
  while (count >= SHA_BLOCKSIZE)
    {
      local_memcpy (sha_info_data_hw, buffer, SHA_BLOCKSIZE);
      sha_transform (sha_info_digest);
      buffer += SHA_BLOCKSIZE;
      count -= SHA_BLOCKSIZE;
    }
  local_memcpy (sha_info_data_hw, buffer, count);
}
...
sha_final (INT32 sha_info_digest[5])
{
  ...
  lo_bit_count = sha_info_count_lo_hw;
  hi_bit_count = sha_info_count_hi_hw;
  count = (int) ((lo_bit_count >> 3) & 0x3f);
  sha_info_data_hw[count++] = 0x80;
  if (count > 56)
    {
      local_memset (sha_info_data_hw, 0, 64 - count, count);
      sha_transform (sha_info_digest);
      local_memset (sha_info_data_hw, 0, 56, 0);
    }
  else
    {
      local_memset (sha_info_data_hw, 0, 56 - count, count);
    }
  sha_info_data_hw[14] = hi_bit_count;
  sha_info_data_hw[15] = lo_bit_count;
  sha_transform (sha_info_digest);
}

/* compute the SHA digest of a FILE stream */
void sha_main_hw (BYTE indata[VSIZE*BLOCK_SIZE], BYTE digest_out[20])
{
  int i, j;
  const BYTE *p;
