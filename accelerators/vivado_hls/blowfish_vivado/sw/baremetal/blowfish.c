...
#define DEV_NAME "sld,blowfish_vivado"

/* <<--params-->> */
const int32_t blowfish_n = 10;
...
static void init_buf (token_t *in, token_t * gold)
{
	...
	for (i = 0; i < blowfish_n; i++)
            blowfish_main_sw((unsigned char *)&in[i*in_words_adj], (unsigned char *)&gold[i*out_words_adj]);
...
int main(int argc, char * argv[])
{
	...
	if (DMA_WORD_PER_BEAT(sizeof(token_t)) == 0) {
		...
		in_words_adj = round_up(blowfish_size, DMA_WORD_PER_BEAT(sizeof(token_t)));
		out_words_adj = round_up(blowfish_size, DMA_WORD_PER_BEAT(sizeof(token_t)));
	}
	...
	ndev = probe(&espdevs, VENDOR_SLD, SLD_BLOWFISH, DEV_NAME);
	if (ndev == 0) {
	...
	for (n = 0; n < ndev; n++) {
		...
		{
			print_time_us(total_time_hw[cnt]/(blowfish_n));
			printf("\n");
		}
		aligned_free(ptable);
		aligned_free(mem);
