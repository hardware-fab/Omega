...
int main(int argc, char **argv) {
    ...
    /* <<--params-->> */
	 const unsigned blowfish_n = 10;
	 const unsigned blowfish_size = 5200;
    ...
    in_words_adj = round_up(blowfish_size, VALUES_PER_WORD);
    out_words_adj = round_up(blowfish_size, VALUES_PER_WORD);
    in_size = in_words_adj * (blowfish_n);
    out_size = out_words_adj * (blowfish_n);
    ...
    // Prepare input data
    for(unsigned i = 0; i < blowfish_n; i++)
        for(unsigned j = 0; j < blowfish_size; j++)
            inbuff[i * in_words_adj + j] = (word_t) (31*i + 47*j + 191)%255;
    ...
    // Set golden output
    for (unsigned i = 0; i < blowfish_n; i++)
        blowfish_main_sw((unsigned char *)&inbuff[i*in_words_adj], (unsigned char *)&outbuff_gold[i*out_words_adj]);
