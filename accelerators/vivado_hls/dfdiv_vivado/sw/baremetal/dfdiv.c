...
#define DEV_NAME "sld,dfdiv_vivado"
...
static void init_buf (token_t *in, token_t * gold)
{
	...
	for (i = 0; i < dfdiv_n; i++)
		for (j = 0; j < dfdiv_in; j++)
			in[i * in_words_adj + j] = (token_t) j*1.78 + i*47.99 + 71.761;

	//Time measurement for software execution
	start_time = custom_gettime_nano();

	for (i = 0; i < dfdiv_n; i++)
    	float64_div_sw((float64 *)&in[i*in_words_adj], (float64 *)&gold[i*out_words_adj]);
...
int main(int argc, char * argv[])
{
	...
	if (DMA_WORD_PER_BEAT(sizeof(token_t)) == 0) {
		...
		in_words_adj = round_up(dfdiv_in, DMA_WORD_PER_BEAT(sizeof(token_t)));
		out_words_adj = round_up(dfdiv_out, DMA_WORD_PER_BEAT(sizeof(token_t)));
	}
	...
	ndev = probe(&espdevs, VENDOR_SLD, SLD_DFDIV, DEV_NAME);
	if (ndev == 0) {
	...
	for (n = 0; n < ndev; n++) {
		...
		{
			print_time_us(total_time_hw[cnt]/(dfdiv_n));
			printf("\n");
		}
		aligned_free(ptable);
		aligned_free(mem);
