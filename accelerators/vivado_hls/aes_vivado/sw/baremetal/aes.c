...
#define DEV_NAME "sld,aes_vivado"
...
static void init_buf (token_t *in, token_t * gold)
{
	...
	for (i = 0; i < aes_n; i++)
		for (j = 0; j < aes_key+aes_text; j++)
			in[i * in_words_adj + j] = (token_t) (31*j + 47*i + 181)%255;

	//Time measurement for software execution
	start_time = custom_gettime_nano();

	for (i = 0; i < aes_n; i++)
		aes_main_sw((int *)&in[i*in_words_adj], (int *)&gold[i*out_words_adj]);
...
int main(int argc, char * argv[])
{
	...
	if (DMA_WORD_PER_BEAT(sizeof(token_t)) == 0) {
		...
		in_words_adj = round_up(aes_key+aes_text, DMA_WORD_PER_BEAT(sizeof(token_t)));
		out_words_adj = round_up(aes_text, DMA_WORD_PER_BEAT(sizeof(token_t)));
	}
	...
	ndev = probe(&espdevs, VENDOR_SLD, SLD_AES, DEV_NAME);
	if (ndev == 0) {
	...
	for (n = 0; n < ndev; n++) {
		...
		{
			print_time_us(total_time_hw[cnt]/(aes_n));
			printf("\n");
		}
		aligned_free(ptable);
		aligned_free(mem);
