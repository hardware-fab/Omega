...
const int32_t aes_key = 32;
const int32_t aes_n = 100;
const int32_t aes_text = 32;

const int32_t blowfish_n = 4;
const int32_t blowfish_size = 5200;

const int32_t dfadd_out = 1;
const int32_t dfadd_in = 2;
const int32_t dfadd_n = 200;

const int32_t dfdiv_n = 100;
const int32_t dfdiv_in = 2;
const int32_t dfdiv_out = 1;

const int32_t dfmul_out = 1;
const int32_t dfmul_n = 200;
...
const int32_t gsm_n = 80;

const int32_t mips_in = 8;
const int32_t mips_n = 100;
const int32_t mips_out = 8;

const int32_t motion_mvfs = 4;
const int32_t motion_rdbfr = 512;
const int32_t motion_n = 8;
const int32_t motion_pwm = 8;

const int32_t sha_vsize = 2;
const int32_t sha_blocksize = 8192;
const int32_t sha_digest = 20;
const int32_t sha_n = 4;
...
const int32_t sha3_message_len = SHA3_MESSAGE_PADDED_WORDS;
const int32_t sha3_n = 8;
const int32_t sha3_hash_len = SHA3_HASH_WORDS;

const int32_t sizeof_token[N_ACC_TYPES] = {sizeof(token_adpcm_t), sizeof(token_aes_t), sizeof(token_aes256_t), sizeof(token_blowfish_t), sizeof(token_dfadd_t), sizeof(token_dfdiv_t), sizeof(token_dfmul_t), sizeof(token_dfsin_t), sizeof(token_gsm_t), sizeof(token_mips_t), sizeof(token_motion_t), sizeof(token_sha_t),
  sizeof(token_sha3_t)
};

const int32_t batch_bytes[N_ACC_TYPES] = {8000, 128, 16, 5200, 8, 8, 8, 8, 336, 32, 48, 16384, 1088};

//--------------Accelerators------------------------------

void print_acc_names(int * acc_types)
{
  printf("DATE2026-ACC: ");
  for (int acc_i=0; acc_i<SOC_NACC; acc_i++)
  {
    switch (acc_types[acc_i])
    {
      case 0:  printf("adpcm "); break;
      case 1:  printf("aes "); break;
      case 2:  printf("aes256 "); break;
      case 3:  printf("blowfish "); break;
      case 4:  printf("dfadd "); break;
      case 5:  printf("dfdiv "); break;
      case 6:  printf("dfmul "); break;
      case 7:  printf("dfsin "); break;
      case 8:  printf("gsm "); break;
      case 9:  printf("mips "); break;
      case 10: printf("motion "); break;
      case 11: printf("sha "); break;
      case 12: printf("sha3 "); break;
    }
  }
  printf("\n");
}
...
//This function searches for all the accelerators of a given type
int search_dev(struct esp_device ** espdevs, int acc_type)
{
  //printf("Entering search_dev function for acc of type %d\n", acc_type);
    int matches = 0;
    (*espdevs) = (struct esp_device *) aligned_malloc(NACC_MAX * sizeof(struct esp_device));

    for (uint64_t i=0; i<SOC_NACC; i++)
    {
        //Address of the accelerator, computed as the base address of acc0 plus the increment due to its position in the noc
        uint32_t * addr = (uint32_t *) (ACC_BASEADDR + (i*ACC_ADDR_INC));
        //Address of the device id
        uint32_t * devid_ptr = (uint32_t *) ((uint8_t *)addr + ACC_DEVID_OFFSET);
        //The device id corresponds to the lower 16 bits of the register
        unsigned devid = 0x0000ffff &  (*devid_ptr);
        //The vendor corresponds to the highest 16 bits of the register
        unsigned vendor = (0xffff0000 &  (*devid_ptr))>>16;

        //Big IF with all the accelerators, to check whether the device id is correct or not
        if (acc_type == 0 && devid == SLD_ADPCM)
        {
            strcpy((*espdevs)[i].name, DEV_NAME_ADPCM);
        }
        else if (acc_type == 1 && devid == SLD_AES)
        {
            strcpy((*espdevs)[i].name, DEV_NAME_AES);
        }
        else if (acc_type == 2 && devid == SLD_AES256)
        {
          strcpy((*espdevs)[i].name, DEV_NAME_AES256);
        }
        else if (acc_type == 3 && devid == SLD_BLOWFISH)
        {
            strcpy((*espdevs)[i].name, DEV_NAME_BLOWFISH);
        }
        else if (acc_type == 4 && devid == SLD_DFADD)
        {
            strcpy((*espdevs)[i].name, DEV_NAME_DFADD);
        }
        else if (acc_type == 5 && devid == SLD_DFDIV)
        {
            strcpy((*espdevs)[i].name, DEV_NAME_DFDIV);
        }
        else if (acc_type == 6 && devid == SLD_DFMUL)
        {
            strcpy((*espdevs)[i].name, DEV_NAME_DFMUL);
        }
        else if (acc_type == 7 && devid == SLD_DFSIN)
        {
            strcpy((*espdevs)[i].name, DEV_NAME_DFSIN);
        }
        else if (acc_type == 8 && devid == SLD_GSM)
        {
            strcpy((*espdevs)[i].name, DEV_NAME_GSM);
        }
        else if (acc_type == 9 && devid == SLD_MIPS)
        {
            strcpy((*espdevs)[i].name, DEV_NAME_MIPS);
        }
        else if (acc_type == 10 && devid == SLD_MOTION)
        {
            strcpy((*espdevs)[i].name, DEV_NAME_MOTION);
        }
        else if (acc_type == 11 && devid == SLD_SHA)
        {
            strcpy((*espdevs)[i].name, DEV_NAME_SHA);
        }
        else if (acc_type == 12 && devid == SLD_SHA3)
        {
            strcpy((*espdevs)[i].name, DEV_NAME_SHA3);
        }
        else
            continue;
        //Fill the esp_device struct
        (*espdevs)[matches].id = devid;
        (*espdevs)[matches].addr = (long long unsigned) addr;
        (*espdevs)[matches].vendor = vendor;
        matches++;
    }
    return matches;
}
...
void init_buf_all(uint8_t *in_byte, uint8_t *out, unsigned in_words_adj, unsigned acc_type)
{
  int i;
  int j;
  ...
  if(acc_type==0)
  {
    token_adpcm_t * in = (token_adpcm_t *)in_byte;
    for (i = 0; i < adpcm_n; i++)
      for (j = 0; j < adpcm_size; j++)
        in[i * in_words_adj + j] = (token_adpcm_t) j/3;
  }
  else if(acc_type==1)
  {
    token_aes_t * in = (token_aes_t *)in_byte;
    for (i = 0; i < aes_n; i++)
      for (j = 0; j < aes_key+aes_text; j++)
        in[i * in_words_adj + j] = (token_aes_t) (31*j + 47*i + 181)%255;
  }
  else if(acc_type==2)
  {
    token_aes256_t * in = (token_aes256_t *)in_byte;
    for (i = 0; i < aes256_n; i++)
      for (j = 0; j < aes256_blockWords; j++)
        in[i * in_words_adj + j] = (token_aes256_t) aes256_data_in[(i*aes256_blockWords+j)%20];
  }
  else if(acc_type==3)
  {
    token_blowfish_t * in = (token_blowfish_t *)in_byte;
    for (i = 0; i < blowfish_n; i++)
      for (j = 0; j < blowfish_size; j++)
        in[i * in_words_adj + j] = (token_blowfish_t) j;
  }
  else if(acc_type==4)
  {
    token_dfadd_t * in = (token_dfadd_t *)in_byte;
    for (i = 0; i < dfadd_n; i++)
      for (j = 0; j < dfadd_in; j++)
        in[i * in_words_adj + j] = (token_dfadd_t)  j*1.57 + i*78.41 + 13.499;
  }
  else if(acc_type==5)
  {
    token_dfdiv_t * in = (token_dfdiv_t *)in_byte;
    for (i = 0; i < dfdiv_n; i++)
      for (j = 0; j < dfdiv_in; j++)
        in[i * in_words_adj + j] = (token_dfdiv_t) j*1.78 + i*47.99 + 71.761;
  }
  else if(acc_type==6)
  {
    token_dfmul_t * in = (token_dfmul_t *)in_byte;
    for (i = 0; i < dfmul_n; i++)
      for (j = 0; j < dfmul_in; j++)
      {
        if(j==0)
          in[i * in_words_adj + j] = (token_dfmul_t) a_input[i%N_DFMUL];
        else
          in[i * in_words_adj + j] = (token_dfmul_t) b_input[i%N_DFMUL];
      }
  }
  else if(acc_type==7)
  {
    token_dfsin_t * in = (token_dfsin_t *)in_byte;
    for (i = 0; i < dfsin_n; i++)
      for (j = 0; j < dfsin_in; j++)
        in[i * in_words_adj + j] = (token_dfsin_t) test_in[i%N_DFSIN];
  }
  else if(acc_type==8)
  {
    token_gsm_t * in = (token_gsm_t *)in_byte;
    for (i = 0; i < gsm_n; i++)
      for (j = 0; j < gsm_nlen; j++)
        in[i * in_words_adj + j] = (token_gsm_t) inData_gsm[j];
  }
  else if(acc_type==9)
  {
    token_mips_t * in = (token_mips_t *)in_byte;
    for (i = 0; i < mips_n; i++)
      for (j = 0; j < mips_in; j++)
        in[i * in_words_adj + j] = (token_mips_t) A[j];
  }
  else if(acc_type==10)
  {
    token_motion_t * in = (token_motion_t *)in_byte;
    for (i = 0; i < motion_n; i++)
      for (j = 0; j < motion_rdbfr+motion_pwm+motion_mvfs; j++)
      {
        if(j<motion_rdbfr)
          in[i * in_words_adj + j] = (token_motion_t) inRdbfr[4*j] | (token_motion_t) inRdbfr[4*j + 1]<<8 | (token_motion_t) inRdbfr[4*j + 2]<<16 | (token_motion_t) inRdbfr[4*j + 3]<<24;
        else if (j<motion_rdbfr + motion_mvfs)
          in[i * in_words_adj + j] = (token_motion_t) inmvfs[(j-motion_rdbfr)/2][(j-motion_rdbfr)%2];
        else
          in[i * in_words_adj + j] = (token_motion_t) inPMV[(j-motion_rdbfr-motion_mvfs)/(2*2)][((j-motion_rdbfr-motion_mvfs)%(2*2))/2][((j-motion_rdbfr-motion_mvfs)%(2))];
      }
  }
  else if(acc_type==11)
  {
    token_sha_t * in = (token_sha_t *)in_byte;
    for (i = 0; i < sha_n; i++)
      for (j = 0; j < sha_vsize*sha_blocksize; j++)
        in[i * in_words_adj + j] = (token_sha_t) indata[j];
  }
  else if(acc_type==12)
  {
      token_sha3_t * in = (token_sha3_t *)in_byte;
      for (i = 0; i < sha3_n; i++)
          for (j = 0; j < sha3_message_len; j++)
              if(j<SHA3_MESSAGE_WORDS-1)
                  in[i * in_words_adj + j] = sha3_data_in[j];
              else if(j==SHA3_MESSAGE_WORDS-1)
                  in[i * in_words_adj + j] = sha3_data_in[j] | (token_sha3_t) ((token_sha3_t)0x6<<(8*(SHA3_MESSAGE_BYTES%8)));
              else if(j<SHA3_MESSAGE_PADDED_WORDS-1)
                  in[i * in_words_adj + j] = (token_sha3_t) 0x0;
              else if(j==SHA3_MESSAGE_PADDED_WORDS-1)
                  in[i * in_words_adj + j] = (token_sha3_t) 0x8000000000000000;
  }
}
...
void init_size_all(unsigned *in_words_adj, unsigned * out_words_adj, unsigned * in_len, unsigned * out_len, unsigned * in_size, unsigned * out_size, unsigned * out_offset, unsigned * mem_size, int acc_type)
{
  ...
  if(acc_type == 0)
  {
    if (DMA_WORD_PER_BEAT(sizeof(token_adpcm_t)) == 0) {
      *in_words_adj = adpcm_size;
      *out_words_adj = adpcm_size;
    } else {
      *in_words_adj = round_up(adpcm_size, DMA_WORD_PER_BEAT(sizeof(token_adpcm_t)));
      *out_words_adj = round_up(adpcm_size, DMA_WORD_PER_BEAT(sizeof(token_adpcm_t)));
    }
    *in_len = *in_words_adj * (adpcm_n);
    *out_len = *out_words_adj * (adpcm_n);
    *in_size = *in_len * sizeof(token_adpcm_t);
    *out_size = *out_len * sizeof(token_adpcm_t);
    *out_offset  = *in_len;
    *mem_size = (*out_offset * sizeof(token_adpcm_t)) + *out_size;
  }
  else if(acc_type == 1)
  {
    if (DMA_WORD_PER_BEAT(sizeof(token_aes_t)) == 0) {
      *in_words_adj = aes_key+aes_text;
      *out_words_adj = aes_text;
    } else {
      *in_words_adj = round_up(aes_key+aes_text, DMA_WORD_PER_BEAT(sizeof(token_aes_t)));
      *out_words_adj = round_up(aes_text, DMA_WORD_PER_BEAT(sizeof(token_aes_t)));
    }
    *in_len = *in_words_adj * (aes_n);
    *out_len = *out_words_adj * (aes_n);
    *in_size = *in_len * sizeof(token_aes_t);
    *out_size = *out_len * sizeof(token_aes_t);
    *out_offset  = *in_len;
    *mem_size = (*out_offset * sizeof(token_aes_t)) + *out_size;
  }
  else if(acc_type == 2)
  {
    if (DMA_WORD_PER_BEAT(sizeof(token_aes256_t)) == 0) {
      *in_words_adj = aes256_blockWords;
      *out_words_adj = aes256_blockWords;
    } else {
      *in_words_adj = round_up(aes256_blockWords, DMA_WORD_PER_BEAT(sizeof(token_aes256_t)));
      *out_words_adj = round_up(aes256_blockWords, DMA_WORD_PER_BEAT(sizeof(token_aes256_t)));
    }
    *in_len =      *in_words_adj * (aes256_n);
    *out_len =     *out_words_adj * (aes256_n);
    *in_size =     *in_len * sizeof(token_aes256_t);
    *out_size =    *out_len * sizeof(token_aes256_t);
    *out_offset  = *in_len;
    *mem_size = (*out_offset * sizeof(token_aes256_t)) + *out_size;
  }
  else if(acc_type == 3)
  {
    if (DMA_WORD_PER_BEAT(sizeof(token_blowfish_t)) == 0) {
      *in_words_adj = blowfish_size;
      *out_words_adj = blowfish_size;
    } else {
      *in_words_adj = round_up(blowfish_size, DMA_WORD_PER_BEAT(sizeof(token_blowfish_t)));
      *out_words_adj = round_up(blowfish_size, DMA_WORD_PER_BEAT(sizeof(token_blowfish_t)));
    }
    *in_len = *in_words_adj * (blowfish_n);
    *out_len = *out_words_adj * (blowfish_n);
    *in_size = *in_len * sizeof(token_blowfish_t);
    *out_size = *out_len * sizeof(token_blowfish_t);
    *out_offset  = *in_len;
    *mem_size = (*out_offset * sizeof(token_blowfish_t)) + *out_size;
  }
  else if(acc_type == 4)
  {
    if (DMA_WORD_PER_BEAT(sizeof(token_dfadd_t)) == 0) {
      *in_words_adj = dfadd_in;
      *out_words_adj = dfadd_out;
    } else {
      *in_words_adj = round_up(dfadd_in, DMA_WORD_PER_BEAT(sizeof(token_dfadd_t)));
      *out_words_adj = round_up(dfadd_out, DMA_WORD_PER_BEAT(sizeof(token_dfadd_t)));
    }
    *in_len = *in_words_adj * (dfadd_n);
    *out_len = *out_words_adj * (dfadd_n);
    *in_size = *in_len * sizeof(token_dfadd_t);
    *out_size = *out_len * sizeof(token_dfadd_t);
    *out_offset  = *in_len;
    *mem_size = (*out_offset * sizeof(token_dfadd_t)) + *out_size;
  }
  else if(acc_type == 5)
  {
    if (DMA_WORD_PER_BEAT(sizeof(token_dfdiv_t)) == 0) {
      *in_words_adj = dfdiv_in;
      *out_words_adj = dfdiv_out;
    } else {
      *in_words_adj = round_up(dfdiv_in, DMA_WORD_PER_BEAT(sizeof(token_dfdiv_t)));
      *out_words_adj = round_up(dfdiv_out, DMA_WORD_PER_BEAT(sizeof(token_dfdiv_t)));
    }
    *in_len = *in_words_adj * (dfdiv_n);
    *out_len = *out_words_adj * (dfdiv_n);
    *in_size = *in_len * sizeof(token_dfdiv_t);
    *out_size = *out_len * sizeof(token_dfdiv_t);
    *out_offset  = *in_len;
    *mem_size = (*out_offset * sizeof(token_dfdiv_t)) + *out_size;
  }
  else if(acc_type == 6)
  {
    if (DMA_WORD_PER_BEAT(sizeof(token_dfmul_t)) == 0) {
      *in_words_adj = dfmul_in;
      *out_words_adj = dfmul_out;
    } else {
      *in_words_adj = round_up(dfmul_in, DMA_WORD_PER_BEAT(sizeof(token_dfmul_t)));
      *out_words_adj = round_up(dfmul_out, DMA_WORD_PER_BEAT(sizeof(token_dfmul_t)));
    }
    *in_len = *in_words_adj * (dfmul_n);
    *out_len = *out_words_adj * (dfmul_n);
    *in_size = *in_len * sizeof(token_dfmul_t);
    *out_size = *out_len * sizeof(token_dfmul_t);
    *out_offset  = *in_len;
    *mem_size = (*out_offset * sizeof(token_dfmul_t)) + *out_size;
  }
  else if(acc_type == 7)
  {
    if (DMA_WORD_PER_BEAT(sizeof(token_dfsin_t)) == 0) {
      *in_words_adj = dfsin_in;
      *out_words_adj = dfsin_out;
    } else {
      *in_words_adj = round_up(dfsin_in, DMA_WORD_PER_BEAT(sizeof(token_dfsin_t)));
      *out_words_adj = round_up(dfsin_out, DMA_WORD_PER_BEAT(sizeof(token_dfsin_t)));
    }
    *in_len = *in_words_adj * (dfsin_n);
    *out_len = *out_words_adj * (dfsin_n);
    *in_size = *in_len * sizeof(token_dfsin_t);
    *out_size = *out_len * sizeof(token_dfsin_t);
    *out_offset  = *in_len;
    *mem_size = (*out_offset * sizeof(token_dfsin_t)) + *out_size;
  }
  else if(acc_type == 8)
  {
    if (DMA_WORD_PER_BEAT(sizeof(token_gsm_t)) == 0) {
      *in_words_adj = gsm_nlen;
      *out_words_adj = gsm_nlen+gsm_mlen;
    } else {
      *in_words_adj = round_up(gsm_nlen, DMA_WORD_PER_BEAT(sizeof(token_gsm_t)));
      *out_words_adj = round_up(gsm_nlen+gsm_mlen, DMA_WORD_PER_BEAT(sizeof(token_gsm_t)));
    }
    *in_len = *in_words_adj * (gsm_n);
    *out_len = *out_words_adj * (gsm_n);
    *in_size= *in_len * sizeof(token_gsm_t);
    *out_size = *out_len * sizeof(token_gsm_t);
    *out_offset  = *in_len;
    *mem_size = (*out_offset * sizeof(token_gsm_t)) + *out_size;
  }
  else if(acc_type == 9)
  {
    if (DMA_WORD_PER_BEAT(sizeof(token_mips_t)) == 0) {
      *in_words_adj = mips_in;
      *out_words_adj = mips_out;
    } else {
      *in_words_adj = round_up(mips_in, DMA_WORD_PER_BEAT(sizeof(token_mips_t)));
      *out_words_adj = round_up(mips_out, DMA_WORD_PER_BEAT(sizeof(token_mips_t)));
    }
    *in_len = *in_words_adj * (mips_n);
    *out_len = *out_words_adj * (mips_n);
    *in_size = *in_len * sizeof(token_mips_t);
    *out_size = *out_len * sizeof(token_mips_t);
    *out_offset  = *in_len;
    *mem_size = (*out_offset * sizeof(token_mips_t)) + *out_size;
  }
  else if(acc_type == 10)
  {
    if (DMA_WORD_PER_BEAT(sizeof(token_motion_t)) == 0) {
      *in_words_adj = motion_rdbfr+motion_pwm+motion_mvfs;
      *out_words_adj = motion_pwm+motion_mvfs;
    } else {
      *in_words_adj = round_up(motion_rdbfr+motion_pwm+motion_mvfs, DMA_WORD_PER_BEAT(sizeof(token_motion_t)));
      *out_words_adj = round_up(motion_pwm+motion_mvfs, DMA_WORD_PER_BEAT(sizeof(token_motion_t)));
    }
    *in_len = *in_words_adj * (motion_n);
    *out_len = *out_words_adj * (motion_n);
    *in_size = *in_len * sizeof(token_motion_t);
    *out_size = *out_len * sizeof(token_motion_t);
    *out_offset  = *in_len;
    *mem_size = (*out_offset * sizeof(token_motion_t)) + *out_size;
  }
  else if(acc_type == 11)
  {
    if (DMA_WORD_PER_BEAT(sizeof(token_sha_t)) == 0) {
      *in_words_adj = sha_vsize*sha_blocksize;
      *out_words_adj = sha_digest;
    } else {
      *in_words_adj = round_up(sha_vsize*sha_blocksize, DMA_WORD_PER_BEAT(sizeof(token_sha_t)));
      *out_words_adj = round_up(sha_digest, DMA_WORD_PER_BEAT(sizeof(token_sha_t)));
    }
    *in_len = *in_words_adj * (sha_n);
    *out_len = *out_words_adj * (sha_n);
    *in_size = *in_len * sizeof(token_sha_t);
    *out_size = *out_len * sizeof(token_sha_t);
    *out_offset  = *in_len;
    *mem_size = (*out_offset * sizeof(token_sha_t)) + *out_size;
  }
  else if(acc_type == 12)
  {
      if (DMA_WORD_PER_BEAT(sizeof(token_sha3_t)) == 0) {
          *in_words_adj = sha3_message_len;
          *out_words_adj = sha3_hash_len;
      } else {
          *in_words_adj = round_up(sha3_message_len, DMA_WORD_PER_BEAT(sizeof(token_sha3_t)));
          *out_words_adj = round_up(sha3_hash_len, DMA_WORD_PER_BEAT(sizeof(token_sha3_t)));
      }
      *in_len = *in_words_adj * (sha3_n);
      *out_len = *out_words_adj * (sha3_n);
      *in_size = *in_len * sizeof(token_sha3_t);
      *out_size = *out_len * sizeof(token_sha3_t);
      *out_offset  = *in_len;
      *mem_size = (*out_offset * sizeof(token_sha3_t)) + *out_size;
  }
}
...
void config_acc_param(struct esp_device *dev, int acc_type)
{
  ...
  {
    iowrite32(dev, AES_AES_KEY_REG, aes_key);
    iowrite32(dev, AES_AES_N_REG, aes_n);
    iowrite32(dev, AES_AES_TEXT_REG, aes_text);
  }
  else if(acc_type==2)
  {
  ...
  {
    iowrite32(dev, BLOWFISH_BLOWFISH_N_REG, blowfish_n);
    iowrite32(dev, BLOWFISH_BLOWFISH_SIZE_REG, blowfish_size);
  }
  else if(acc_type==4)
  {
  ...
  {
    iowrite32(dev, DFDIV_DFDIV_N_REG, dfdiv_n);
    iowrite32(dev, DFDIV_DFDIV_IN_REG, dfdiv_in);
    iowrite32(dev, DFDIV_DFDIV_OUT_REG, dfdiv_out);
  }
  else if(acc_type==6)
  {
  ...
  {
    iowrite32(dev, MIPS_MIPS_IN_REG, mips_in);
    iowrite32(dev, MIPS_MIPS_N_REG, mips_n);
    iowrite32(dev, MIPS_MIPS_OUT_REG, mips_out);
  }
  else if(acc_type==10)
  {
  ...
  {
    iowrite32(dev, SHA_SHA_VSIZE_REG, sha_vsize);
    iowrite32(dev, SHA_SHA_BLOCKSIZE_REG, sha_blocksize);
    iowrite32(dev, SHA_SHA_DIGEST_REG, sha_digest);
    iowrite32(dev, SHA_SHA_N_REG, sha_n);
    }
  else if(acc_type==12)
  {
...
{
    if(acc_type == 0)
        *acc_batch_size = adpcm_n;
    else if(acc_type == 1)
        *acc_batch_size = aes_n;
    else if(acc_type == 2)
        *acc_batch_size = aes256_n;
    else if(acc_type == 3)
        *acc_batch_size = blowfish_n;
    else if(acc_type == 4)
        *acc_batch_size = dfadd_n;
    else if(acc_type == 5)
        *acc_batch_size = dfdiv_n;
    else if(acc_type == 6)
        *acc_batch_size = dfmul_n;
    else if(acc_type == 7)
        *acc_batch_size = dfsin_n;
    else if(acc_type == 8)
        *acc_batch_size = gsm_n;
    else if(acc_type == 9)
        *acc_batch_size = mips_n;
    else if(acc_type == 10)
        *acc_batch_size = motion_n;
    else if(acc_type == 11)
        *acc_batch_size = sha_n;
    else if(acc_type == 12)
        *acc_batch_size = sha3_n;
}

// This function takes the address of an accelerator and returns its NoC position
// (counting accelerator first from left to right and then from top to bottom)
uint8_t get_index_from_address(uint32_t address)
{
  uint16_t address_low = (uint16_t)(address & 0x0000FFFF);
  uint8_t index = address_low >> 8;
  return index;
}
...
int print_results(int acc_type, uint8_t *gold_in, uint8_t *out_in, unsigned out_words_adj, uint8_t *input_in)
{
  ...
  if(acc_type==0)
  {
    token_adpcm_t *gold = (token_adpcm_t *)gold_in;
    token_adpcm_t *out = (token_adpcm_t *)out_in;
    printf("\n----------Results adpcm:---------\n");
    for (i = 0; i < adpcm_n; i++)
    {
      if(i==0)
  ...
  {
    token_aes_t *gold = (token_aes_t *)gold_in;
    token_aes_t *out = (token_aes_t *)out_in;
    printf("\n----------Results aes:---------\n");
    for (i = 0; i < aes_n; i++)
    {
      if(i==0)
  ...
  {
    token_aes256_t *gold = (token_aes256_t *)gold_in;
    token_aes256_t *out = (token_aes256_t *)out_in;
    printf("\n----------Results:---------\n");
    for (i = 0; i < aes256_n; i++)
    {
      printf("\nBatch %d:\n", i);
  ...
  {
    token_blowfish_t *gold = (token_blowfish_t *)gold_in;
    token_blowfish_t *out = (token_blowfish_t *)out_in;
    for (i = 0; i < blowfish_n; i++)
      for (j = 0; j < blowfish_size; j++)
      {
        if(i==0)
  ...
  {
    token_dfadd_t *gold = (token_dfadd_t *)gold_in;
    token_dfadd_t *out = (token_dfadd_t *)out_in;
    printf("\n----------Results dfadd:---------\n");
    for (i = 0; i < dfadd_n; i++)
    {
      printf("\nBatch %d:\n", i);
  ...
  {
    token_dfdiv_t *gold = (token_dfdiv_t *)gold_in;
    token_dfdiv_t *out = (token_dfdiv_t *)out_in;
    printf("\n----------Results dfdiv:---------\n");
    for (i = 0; i < dfdiv_n; i++)
    {
      printf("\nBatch %d:\n", i);
      for (j = 0; j < dfdiv_out; j++)
      {
        printf("%d    -    gold = %016llx     out = %016llx\n", j, gold[i * out_words_adj + j], out[i * out_words_adj + j]);
        if (gold[i * out_words_adj + j] != out[i * out_words_adj + j])
          errors++;
  ...
  {
    token_dfmul_t *gold = (token_dfmul_t *)gold_in;
    token_dfmul_t *out = (token_dfmul_t *)out_in;
    printf("\n----------Results dfmul:---------\n");
    for (i = 0; i < dfmul_n; i++)
    {
      printf("\nBatch %d:\n", i);
  ...
  {
    token_dfsin_t *gold = (token_dfsin_t *)gold_in;
    token_dfsin_t *out = (token_dfsin_t *)out_in;

    token_dfsin_t *in = (token_dfsin_t *)input_in;
    ...
    for (i = 0; i < dfsin_n; i++)
    {
      ...
      {
        printf("%d    -    in = %016llx     gold = %016llx     out = %016llx\n", j, in[i * out_words_adj + j], gold[i * out_words_adj + j], out[i * out_words_adj + j]);
        //printf("%d    -    gold = %016llx     out = %016llx\n", j, gold[i * out_words_adj + j], out[i * out_words_adj + j]);
        //if (gold[i * out_words_adj + j] != out[i * out_words_adj + j])
        //  errors++;
      }
  ...
  {
    token_gsm_t *gold = (token_gsm_t *)gold_in;
    token_gsm_t *out = (token_gsm_t *)out_in;
    printf("\n----------Results gsm:---------\n");
    for (i = 0; i < gsm_n; i++)
    {
      if(i==0)
  ...
  {
    token_mips_t *gold = (token_mips_t *)gold_in;
    token_mips_t *out = (token_mips_t *)out_in;
    printf("\n----------Results mips:---------\n");
    for (i = 0; i < mips_n; i++)
    {
      printf("\nBatch %d:\n", i);
      for (j = 0; j < mips_out; j++)
      {
        printf("%d    -    gold = %d     out = %d\n", j, gold[i * out_words_adj + j], out[i * out_words_adj + j]);
        if (gold[i * out_words_adj + j] != out[i * out_words_adj + j])
          errors++;
  ...
  {
    token_motion_t *gold = (token_motion_t *)gold_in;
    token_motion_t *out = (token_motion_t *)out_in;
    printf("\n----------Results motion:---------\n");
    for (i = 0; i < motion_n; i++)
    {
      printf("\nBatch %d:\n", i);
  ...
  {
    token_sha_t *gold = (token_sha_t *)gold_in;
    token_sha_t *out = (token_sha_t *)out_in;
    printf("\n----------Results sha:---------\n");
    for (i = 0; i < sha_n; i++)
    {
      printf("\nBatch %d:\n", i);
      for (j = 0; j < sha_digest; j++)
      {
        printf("%d    -    gold = %02x     out = %02x\n", j, (unsigned char) gold[i * out_words_adj + j], (unsigned char) out[i * out_words_adj + j]);
        if (gold[i * out_words_adj + j] != out[i * out_words_adj + j])
          errors++;
      }
    }
    ...
    {
        token_sha3_t *gold = (token_sha3_t *)gold_in;
    token_sha3_t *out = (token_sha3_t *)out_in;
        printf("\n----------Results:---------\n");
        for (i = 0; i < sha3_n; i++)
        {
            printf("\nBatch %d:\n", i);
...
const float64 z_output[N_DFMUL] = {
  0xFFFFFFFFFFFFFFFFULL,  /* nan */
  0x7FFF000000000000ULL,  /* nan */
...
const float64 test_out[N_DFSIN] = {
...
const unsigned char inRdbfr[N_MOTION] = {
...
const INT32 outData_SHA[5] =
  { 0x006a5a37UL, 0x93dc9485UL, 0x2c412112UL, 0x63f7ba43UL, 0xad73f922UL };


const token_aes256_t aes256_data_in[20]  = {0xb792defe83b94d76, 0xa07444e3190b4586, 0x583d7a9a1b90cbbb, 0xd01ec550e708a4c1, 0x7783d0fa3c1e700d, 0x050f9f1e1ae4a5d2, 0x3e5f3059f0fb15f5, 0x3c355d233d01e596, 0x3bc70a78e57a8521,
...
const token_aes256_t aes256_data_out[20] = {0x1ce62c2358607c05, 0x9a23a49ef1da13e7, 0x11d10e65e69ce318, 0xf043761f673ac081, 0x418d3bf0acda9afa, 0xb8fb8435faba28b7, 0xf76a94b7d0ef12ed, 0xa547b71cf4eb93a2, 0x698932f9a9c1d7a7,
