...
use work.socmap.all; --GM change: I need this library for roundtrip monitoring
...
-- <<architecture>>
  ...
  --GM change: custom monitor for the roundtrip time
  component roundtrip_time_monitor
    generic (
    ...
    port (
      clk_i                     :   in  std_logic;
      rstn_i                    :   in  std_logic;
      acc_req_valid_i           :   in  std_logic;
      mem_data_valid_i          :   in  std_logic;
      batch_length_i            :   in  std_logic_vector(MON_ACC_RTT_SIZE-1 downto 0);
  ...
  --GM change: batch length
  signal batch_len : std_logic_vector(MON_ACC_RTT_SIZE-1 downto 0);
...
begin
  ...
  --GM change: a monitor for the roundtrip time
  rtt_mon_inst : roundtrip_time_monitor
  generic map (
    DATA_WIDTH    => MON_ACC_RTT_SIZE,
    LOG2_N_CORES  => log2_n_cores
  )
  port map (
    clk_i                   => clk,
    rstn_i                  => rst,
    acc_req_valid_i         => dma_read_ctrl_valid,
    mem_data_valid_i        => dma_read_chnl_valid,
    batch_length_i          => batch_len,
    mon_acc_roundtrip_time  => mon_acc.roundtrip_time
  );
