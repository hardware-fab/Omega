...
def write_acc_interface(f, acc, dma_width, datatype, rst, is_vivadohls_if, is_catapulthls_cxx_if, is_catapulthls_sysc_if, is_rtl):
  ...
  if is_catapulthls_cxx_if:
    ...
    f.write("      ap_idle                    : out std_ulogic;\n")
    f.write("      ap_ready                   : out std_ulogic;\n")
    f.write("      dma_read_ctrl_ready        : in  std_logic;\n")
    f.write("      dma_read_ctrl_valid        : out std_logic;\n")
    f.write("      dma_read_ctrl_data         : out std_logic_vector (" + str(95) + " downto 0);\n")
    f.write("      dma_read_chnl_ready        : out std_logic;\n")
    f.write("      dma_read_chnl_valid        : in  std_logic;\n")
    f.write("      dma_read_chnl_data         : in  std_logic_vector (" + str(dma_width - 1) + " downto 0);\n")
    f.write("      dma_write_ctrl_ready       : in  std_logic;\n")
    f.write("      dma_write_ctrl_valid       : out std_logic;\n")
    f.write("      dma_write_ctrl_data        : out std_logic_vector (" + str(95) + " downto 0);\n")
    f.write("      dma_write_chnl_ready       : in  std_logic;\n")
    f.write("      dma_write_chnl_valid       : out std_logic;\n")
...
def write_ap_acc_signals(f):
  f.write("\n\n")
  f.write("--Batch size must be divided for the number of modules\n")
  f.write("type conf_batch_size_type is array (0 to N_CORES-1) of std_logic_vector(31 downto 0);\n")
  f.write("signal multi_conf_batch_size : conf_batch_size_type;\n")
  f.write("\n")
  f.write("--Parallelized ap signals\n")
  f.write("signal ap_rst : std_ulogic;\n")
  f.write("signal ap_start : std_logic_vector(N_CORES-1 downto 0);\n")
  f.write("signal ap_done, ap_done_saved, ap_done_saved_r : std_logic_vector(N_CORES-1 downto 0);\n")
  f.write("signal ap_idle : std_logic_vector(N_CORES-1 downto 0);\n")
  f.write("signal ap_ready : std_logic_vector(N_CORES-1 downto 0);\n")
  f.write("\n")
  f.write("--Start FSM\n")
  f.write("type start_state_t is (aps_idle, aps_starting, aps_running, aps_wait_for_completion);\n")
  f.write("type start_state_array_t is array (0 to N_CORES-1) of start_state_t;\n")
  f.write("signal ap_state, ap_next : start_state_array_t;\n")
  f.write("\n")
  f.write("--DMA read control\n")
  f.write("signal multi_dma_read_ctrl_data : multi_dma_ctrl_data_t(N_CORES-1 downto 0);\n")
  f.write("signal multi_dma_read_ctrl_valid : std_logic_vector(N_CORES-1 downto 0);\n")
  f.write("signal multi_dma_read_ctrl_ready : std_logic_vector(N_CORES-1 downto 0);\n")
  f.write("--DMA write control\n")
  f.write("signal multi_dma_write_ctrl_data : multi_dma_ctrl_data_t(N_CORES-1 downto 0);\n")
  f.write("signal multi_dma_write_ctrl_valid : std_logic_vector(N_CORES-1 downto 0);\n")
  f.write("signal multi_dma_write_ctrl_ready : std_logic_vector(N_CORES-1 downto 0);\n")
  f.write("--DMA read channel\n")
  f.write("signal multi_dma_read_chnl_data : multi_dma_chnl_data_t(N_CORES-1 downto 0);\n")
  f.write("signal multi_dma_read_chnl_valid : std_logic_vector(N_CORES-1 downto 0);\n")
  f.write("signal multi_dma_read_chnl_ready : std_logic_vector(N_CORES-1 downto 0);\n")
  f.write("--DMA write channel\n")
  f.write("signal multi_dma_write_chnl_data : multi_dma_chnl_data_t(N_CORES-1 downto 0);\n")
  f.write("signal multi_dma_write_chnl_valid : std_logic_vector(N_CORES-1 downto 0);\n")
  f.write("signal multi_dma_write_chnl_ready : std_logic_vector(N_CORES-1 downto 0);\n")
  f.write("\n")
  f.write("--Useful wires\n")
  ...
  f.write("signal multi_dma_read_ctrl_data_reg : multi_dma_ctrl_data_t(N_CORES-1 downto 0);\n")
  f.write("signal multi_dma_write_ctrl_data_reg : multi_dma_ctrl_data_t(N_CORES-1 downto 0);\n")
  f.write("\n")

def write_acc_port_map(f, acc, dma_width, datatype, rst, is_noc_interface, is_vivadohls_if, is_catapulthls_cxx_if, is_catapulthls_sysc_if, is_rtl_if):
  ...
  if is_catapulthls_cxx_if:
  ...
  elif(is_vivadohls_if or is_rtl_if):
    f.write("    port map(\n")
    batch_len = ""
    for param in acc.param:
      if not param.readonly:
        ...
        #GM change: this is the batch length, and needs to be treated differently
        elif param.name.endswith("_n"):
          f.write("      conf_info_" + param.name + spacing + " => multi_conf_batch_size(i),\n")
          batch_len = param.name
        else:
          f.write("      conf_info_" + param.name + spacing + " => " + "conf_info_" + param.name +",\n")
    f.write("      ap_clk                     => clk,\n")
    f.write("      ap_rst                     => ap_rst,\n")
    f.write("      ap_start                   => ap_start(i),\n")
    if(is_vivadohls_if): #if vivado hls
      f.write("      load_ctrl_ap_vld           => multi_dma_read_ctrl_valid(i),\n")
      f.write("      load_ctrl_ap_ack           => multi_dma_read_ctrl_ready(i),\n")
      f.write("      load_ctrl                  => multi_dma_read_ctrl_data(i),\n")
      f.write("      store_ctrl_ap_vld          => multi_dma_write_ctrl_valid(i),\n")
      f.write("      store_ctrl_ap_ack          => multi_dma_write_ctrl_ready(i),\n")
      f.write("      store_ctrl                 => multi_dma_write_ctrl_data(i),\n")
      if datatype == 'float':
        f.write("      in1_word_empty_n         => multi_dma_read_chnl_valid(i),\n")
        f.write("      in1_word_read            => multi_dma_read_chnl_ready(i),\n")
        f.write("      in1_word_dout            => multi_dma_read_chnl_data(i),\n")
        f.write("      out_word_write           => multi_dma_write_chnl_valid(i),\n")
        f.write("      out_word_full_n          => multi_dma_write_chnl_ready(i),\n")
        f.write("      out_word_din             => multi_dma_write_chnl_data(i),\n")
      else:
        f.write("      in1_word_V_empty_n         => multi_dma_read_chnl_valid(i),\n")
        f.write("      in1_word_V_read            => multi_dma_read_chnl_ready(i),\n")
        f.write("      in1_word_V_dout            => multi_dma_read_chnl_data(i),\n")
        f.write("      out_word_V_write           => multi_dma_write_chnl_valid(i),\n")
        f.write("      out_word_V_full_n          => multi_dma_write_chnl_ready(i),\n")
        f.write("      out_word_V_din             => multi_dma_write_chnl_data(i),\n")
    #end if vivado hls
    else: #if rtl
      f.write("      dma_read_ctrl_valid          => multi_dma_read_ctrl_valid(i),\n")
      f.write("      dma_read_ctrl_ready          => multi_dma_read_ctrl_ready(i),\n")
      f.write("      dma_read_ctrl_data           => multi_dma_read_ctrl_data(i),\n")
      f.write("      dma_write_ctrl_valid         => multi_dma_write_ctrl_valid(i),\n")
      f.write("      dma_write_ctrl_ready         => multi_dma_write_ctrl_ready(i),\n")
      f.write("      dma_write_ctrl_data          => multi_dma_write_ctrl_data(i),\n")
      f.write("      dma_read_chnl_valid          => multi_dma_read_chnl_valid(i),\n")
      f.write("      dma_read_chnl_ready          => multi_dma_read_chnl_ready(i),\n")
      f.write("      dma_read_chnl_data           => multi_dma_read_chnl_data(i),\n")
      f.write("      dma_write_chnl_valid         => multi_dma_write_chnl_valid(i),\n")
      f.write("      dma_write_chnl_ready         => multi_dma_write_chnl_ready(i),\n")
      f.write("      dma_write_chnl_data          => multi_dma_write_chnl_data(i),\n")
    #end if rtl
    f.write("      ap_done                    => ap_done(i),\n")
    f.write("      ap_idle                    => ap_idle(i),\n")
    f.write("      ap_ready                   => ap_ready(i)\n")
    f.write("      );\n")
    f.write("    " + "end generate cores_gen;\n")
    f.write("\n")
    f.write("  --DMA read control\n")
    f.write("  dma_ctrl_read_inst: dma_ctrl_mux\n")
    f.write("    generic map(\n")
    f.write("      N_CORES       => N_CORES\n")
    f.write("    )\n")
    f.write("    port map(\n")
    f.write("      clk                           => clk,\n")
    f.write("      rst                           => ap_rst,\n")
    f.write("\n")
    f.write("      multi_dma_ctrl_data_acc       => multi_dma_read_ctrl_data,\n")
    f.write("      multi_dma_ctrl_valid_acc      => multi_dma_read_ctrl_valid,\n")
    f.write("      multi_dma_ctrl_ready_acc      => multi_dma_read_ctrl_ready,\n")
    f.write("\n")
    f.write("      dma_ctrl_data_mem             => dma_read_ctrl_data,\n")
    f.write("      dma_ctrl_valid_mem            => dma_read_ctrl_valid,\n")
    f.write("      dma_ctrl_ready_mem            => dma_read_ctrl_ready,\n")
    f.write("\n")
    f.write("      multi_dma_ctrl_data_reg       => multi_dma_read_ctrl_data_reg,\n")
    f.write("\n")
    f.write("      multi_ap_done                 => ap_done_saved_r\n")
    f.write("    );\n")
    f.write("\n")
    f.write("  --DMA write control\n")
    f.write("  dma_ctrl_write_inst: dma_ctrl_mux\n")
    f.write("    generic map(\n")
    f.write("      N_CORES       => N_CORES\n")
    f.write("    )\n")
    f.write("    port map(\n")
    ...
    f.write("      rst                           => ap_rst,\n")
    f.write("\n")
    f.write("      multi_dma_ctrl_data_acc       => multi_dma_write_ctrl_data,\n")
    f.write("      multi_dma_ctrl_valid_acc      => multi_dma_write_ctrl_valid,\n")
    f.write("      multi_dma_ctrl_ready_acc      => multi_dma_write_ctrl_ready,\n")
    f.write("\n")
    f.write("      dma_ctrl_data_mem             => dma_write_ctrl_data,\n")
    f.write("      dma_ctrl_valid_mem            => dma_write_ctrl_valid,\n")
    f.write("      dma_ctrl_ready_mem            => dma_write_ctrl_ready,\n")
    f.write("\n")
    f.write("      multi_dma_ctrl_data_reg       => multi_dma_write_ctrl_data_reg,\n")
    f.write("\n")
    f.write("      multi_ap_done                 => ap_done_saved_r\n")
    f.write("    );\n")
    f.write("\n")
    f.write("  --DMA read channel\n")
    f.write("  dma_chnl_read_inst:  dma_multi_read\n")
    f.write("  generic map(\n")
    f.write("    N_CORES       => N_CORES\n")
    f.write("  )\n")
    f.write("  port map(\n")
    ...
    f.write("    rst                           => ap_rst,\n")
    f.write("\n")
    f.write("    dma_chnl_data_mem             => dma_read_chnl_data,\n")
    f.write("    dma_chnl_valid_mem            => dma_read_chnl_valid,\n")
    f.write("    dma_chnl_ready_mem            => dma_read_chnl_ready,\n")
    f.write("\n")
    f.write("    multi_dma_chnl_data_acc       => multi_dma_read_chnl_data,\n")
    f.write("    multi_dma_chnl_valid_acc      => multi_dma_read_chnl_valid,\n")
    f.write("    multi_dma_chnl_ready_acc      => multi_dma_read_chnl_ready,\n")
    f.write("\n")
    f.write("    multi_dma_ctrl_data_reg       => multi_dma_read_ctrl_data_reg,\n")
    f.write("\n")
    f.write("      multi_ap_done                 => ap_done_saved_r\n")
    ...
    f.write("\n")
    f.write("  --DMA write channel\n")
    f.write("  dma_chnl_write_inst:  dma_multi_write\n")
    f.write("  generic map(\n")
    f.write("    N_CORES       => N_CORES\n")
    f.write("  )\n")
    f.write("  port map(\n")
    ...
    f.write("    rst                           => ap_rst,\n")
    f.write("\n")
    f.write("    dma_chnl_data_mem             => dma_write_chnl_data,\n")
    f.write("    dma_chnl_valid_mem            => dma_write_chnl_valid,\n")
    f.write("    dma_chnl_ready_mem            => dma_write_chnl_ready,\n")
    f.write("\n")
    f.write("    multi_dma_chnl_data_acc       => multi_dma_write_chnl_data,\n")
    f.write("    multi_dma_chnl_valid_acc      => multi_dma_write_chnl_valid,\n")
    f.write("    multi_dma_chnl_ready_acc      => multi_dma_write_chnl_ready,\n")
    f.write("\n")
    f.write("    multi_dma_ctrl_data_reg       => multi_dma_write_ctrl_data_reg,\n")
    f.write("\n")
    f.write("      multi_ap_done                 => ap_done_saved_r\n")
    ...
    f.write("\n")
    f.write("  --Done control\n")
    f.write("  and_done: process(ap_done, ap_done_saved_r)\n")
    f.write("    variable done_temp : std_logic;\n")
    f.write("  begin\n")
    f.write("    done_temp := '1';\n")
    f.write("    ap_done_saved <= ap_done_saved_r;\n")
    f.write("    --Each done remain high for just one clock cycle: here they are preserved for the global done to be issued to the cpu\n")
    f.write("    for i in 0 to N_CORES-1 loop\n")
    f.write("      if ap_done(i) = '1' then\n")
    f.write("        ap_done_saved(i) <= '1';\n")
    f.write("      end if;\n")
    f.write("    end loop;\n")
    f.write("    --The done flag is issued when every core is done\n")
    f.write("    for i in 0 to N_CORES-1 loop\n")
    f.write("      done_temp := done_temp and ap_done_saved_r(i);\n")
    f.write("    end loop;\n")
    f.write("    acc_done <= done_temp;\n")
    f.write("    --If done is issued, reset the done registers (so that global done remains high for just one clock cycle)\n")
    f.write("    if done_temp = '1' then\n")
    f.write("      ap_done_saved <= (others => '0');\n")
    f.write("    end if;\n")
    f.write("  end process and_done;\n")
    f.write("\n")
    f.write("  -- START FSM\n")
    f.write("  fsm_gen: for i in 0 to N_CORES-1 generate\n")
    f.write("    ap_acc_fsm: process (ap_state(i), conf_done, ap_idle(i), ap_ready(i), ap_done(i)) is\n")
    f.write("    begin  -- process ap_acc_fsm\n")
    f.write("      ap_next(i) <= ap_state(i);\n")
    f.write("      ap_start(i) <= '0';\n")
    f.write("\n")
    f.write("      case ap_state(i) is\n")
    f.write("\n")
    f.write("        when aps_idle =>\n")
    f.write("          if ap_idle(i) = '1' and conf_done = '1' then\n")
    f.write("            ap_next(i) <= aps_starting;\n")
    f.write("          end if;\n")
    f.write("\n")
    f.write("        when aps_starting =>\n")
    f.write("          ap_start(i) <= '1';\n")
    f.write("          if ap_ready(i) = '1' then\n")
    f.write("            ap_next(i) <= aps_wait_for_completion;\n")
    f.write("          elsif ap_idle(i) = '0' then\n")
    f.write("            ap_next(i) <= aps_running;\n")
    f.write("          end if;\n")
    f.write("\n")
    f.write("        when aps_running =>\n")
    f.write("          ap_start(i) <= '1';\n")
    f.write("          if ap_done(i) = '1' then\n")
    f.write("            ap_next(i) <= aps_idle;\n")
    f.write("          elsif ap_ready(i) = '1' then\n")
    f.write("            ap_next(i) <= aps_wait_for_completion;\n")
    f.write("          end if;\n")
    f.write("\n")
    ...
    f.write("        ap_state <= (others => aps_idle);\n")
    f.write("        ap_done_saved_r <= (others => '0');\n")
    f.write("      else\n")
    f.write("        ap_state <= ap_next;\n")
    f.write("        ap_done_saved_r <= ap_done_saved;\n")
    f.write("      end if;\n")
    f.write("    end if;\n")
    f.write("  end process ap_acc_state_update;\n")
    f.write("\n")
    f.write("  --Divide the number of batches for each core\n")
    f.write("  --(TODO: this works only if N_CORES is a power of two. We may decide to extend this functionality for every value of N_CORES)\n")
    f.write("  batch_division: process (conf_info_" + batch_len + ")\n")
    f.write("  variable remainder_v : unsigned(LOG2_N_CORES downto 0); --LOG2_N_CORES can actually be zero\n")
    f.write("  variable quotient_v : unsigned(conf_info_" + batch_len + "'RANGE);\n")
    f.write("  begin\n")
    f.write("    if LOG2_N_CORES = 0 then\n")
    f.write("      remainder_v := (others => '0');\n")
    f.write("    else\n")
    f.write("      remainder_v := '0' & unsigned(conf_info_" + batch_len + "(LOG2_N_CORES-1 downto 0));\n")
    f.write("    end if;\n")
    f.write("    quotient_v := shift_right(unsigned(conf_info_" + batch_len + "), LOG2_N_CORES);\n")
    f.write("    for i in 0 to N_CORES-1 loop\n")
    f.write("      if i < remainder_v then\n")
    f.write("        multi_conf_batch_size(i) <= std_logic_vector(quotient_v+1);\n")
    f.write("      else\n")
    f.write("        multi_conf_batch_size(i) <= std_logic_vector(quotient_v);\n")
    f.write("      end if;\n")
    f.write("    end loop;\n")
    f.write("  end process batch_division;\n")
    f.write("\n")
    f.write("  --Division of control bus in size, length and index information\n")
      ...
      for acc in accelerator_list:
        for impl in acc.hlscfg:
          ...
          if acc.hls_tool == 'stratus_hls':
            ...
            f.write("    port (\n")
            write_acc_interface(f, acc, dma_width, impl.datatype, "rst", False, False, False, False)
          elif acc.hls_tool == 'rtl':
            f.write("  component " + acc.name + "_" + impl.name + "\n")
            f.write("    port (\n")
            write_acc_interface(f, acc, dma_width, impl.datatype, "ap_rst", False, False, False, True)
          elif acc.hls_tool == 'catapult_hls_cxx':
            f.write("  component " + acc.name + "_" + impl.name + "\n")
            f.write("    port (\n")
            write_acc_interface(f, acc, dma_width, impl.datatype, "rst", False, True, False, False)
          elif acc.hls_tool == 'catapult_hls_sysc':
            f.write("  component " + acc.name + "_" + impl.name + "\n")
            f.write("    port (\n")
            write_acc_interface(f, acc, dma_width, impl.datatype, "rst", False, False, True, False)
          else:
            f.write("  component " + acc.name + "_" + impl.name + "_top\n")
            f.write("    port (\n")
            write_acc_interface(f, acc, dma_width, impl.datatype, "ap_rst", True, False, False, False)
          f.write("    );\n")
  ...
  with open(template_dir + '/genacc.vhd', 'r') as ftemplate:
    for tline in ftemplate:
      ...
      for acc in accelerator_list:
        ...
        f.write("      N_CORES            : integer := 1;\n")
        f.write("      LOG2_N_CORES       : integer := 0\n")
        f.write("    );\n")
        f.write("\n")
        f.write("    port (\n")
        write_acc_interface(f, acc, dma_width, "", "acc_rst", False, False, False, False)
        f.write("    );\n")
        f.write("  end component;\n\n")
        f.write("\n")
      ...
      for acc in accelerator_list:
        if acc.hls_tool == 'catapult_hls_cxx':
          ...
          for impl in acc.hlscfg:
            ...
            f.write("  impl_" + impl.name + "_gen: if hls_conf = HLSCFG_" + acc.name.upper() + "_" + impl.name.upper() + " generate\n")
            f.write("    " + acc.name + "_" + impl.name + "_i: " + acc.name + "_" + impl.name + "\n")
            write_acc_port_map(f, acc, dma_width, impl.datatype, "rst", False, False, False, True, False)
            f.write("\n\n")
            f.write("  -- CONF_DONE FSM\n")
            f.write("\n")
            f.write("  conf_done_fsm: process (rsc_state, conf_done, conf_info_rsc_ready) is\n")
          ...
          f.write("use work.sld_devices.all;\n")
          f.write("use work.allacc.all;\n\n")
          f.write("--GM change: libraries for parallel execution\n")
          f.write("use ieee.numeric_std.all;\n")
          f.write("use work.parallel_acc_pkg.all;\n")
          f.write("use work.esp_global.all;\n")
          f.write("use work.socmap.all;\n")
          f.write("\n")
          f.write("entity " + acc.name + "_rtl is\n\n")
          f.write("    generic (\n")
          f.write("      hls_conf           : hlscfg_t;\n")
          ...
          f.write("\n")
          f.write("architecture mapping of " + acc.name + "_rtl is\n\n")
          if acc.hls_tool == 'vivado_hls' or acc.hls_tool == 'rtl':
          ...
          for impl in acc.hlscfg:
            ...
            if acc.hls_tool == 'stratus_hls': # or acc.hls_tool == 'rtl':
              f.write("    " + acc.name + "_" + impl.name + "_i: " + acc.name + "_" + impl.name + "\n")
              write_acc_port_map(f, acc, dma_width, impl.datatype, "rst", False, False, False, False, False)
            #Right now, parallel execution has been tested only for vivado hls and rtl accelerators
            elif acc.hls_tool == 'vivado_hls':
              f.write("    " + "cores_gen : for i in 0 to N_CORES-1 generate\n")
              f.write("      " + acc.name + "_" + impl.name + "_top_i: " + acc.name + "_" + impl.name + "_top\n")
              write_acc_port_map(f, acc, dma_width, impl.datatype, "rst", False, True, False, False, False)
            elif acc.hls_tool == 'rtl':
              f.write("    " + "cores_gen : for i in 0 to N_CORES-1 generate\n")
              f.write("      " + acc.name + "_" + impl.name + "_top_i: " + acc.name + "_" + impl.name + "\n")
              write_acc_port_map(f, acc, dma_width, impl.datatype, "rst", False, False, False, False, True)
            else:
              f.write("    " + acc.name + "_" + impl.name + "_top_i: " + acc.name + "_" + impl.name + "_top\n")
              write_acc_port_map(f, acc, dma_width, impl.datatype, "rst", False, True, False, False, False)
            f.write("  end generate impl_" +  impl.name + "_gen;\n\n")
    ...
    for tline in ftemplate:
      if tline.find("-- <<entity>>") >= 0:
        ...
        if is_axi:
          ...
          f.write("      N_CORES        => n_cores,\n")
          f.write("      LOG2_N_CORES   => log2_n_cores\n")
          f.write("    )\n")
          write_acc_port_map(f, acc, dma_width, "", "acc_rst", True, False, False, False, False)
          #Batch length assignment
          for param in acc.param:
            if not param.readonly and param.name.endswith("_n"):
              f.write("\n  batch_len <= bank(" + acc.name.upper() + "_" + param.name.upper() + "_REG)(MON_ACC_RTT_SIZE-1 downto 0);\n\n")
      else:
        f.write(tline)
    ...
    for tline in ftemplate:
      if tline.find("-- <<accelerator-wrappers-gen>>") >= 0:
        for acc in accelerator_list + axi_accelerator_list:
          ...
          f.write("        n_cores        => this_n_cores,\n")
          f.write("        log2_n_cores   => this_log2_n_cores)\n")
          f.write("      port map (\n")
          f.write("        rst               => rstn_tile,\n")
...
#This function generates all the reconfigurable modules necessary for the dynamic partial reconfiguration of accelerators

#These are the level of parallelism that will be generated. Change this list to generate different level of parallelism.
par_lvls = [1, 2, 4]

def gen_tile_dpr(accelerator_list, axi_acceleratorlist, template_dir, out_dir):
  if os.path.exists(out_dir + "/dpr"):
    shutil.rmtree(out_dir + "/dpr")
  os.makedirs(out_dir + "/dpr")
  for acc in accelerator_list:
    for impl in acc.hlscfg:
      for n_cores in par_lvls:
        with open(out_dir + "/dpr/tile_dpr_" + acc.name.upper() + "_PAR" + str(n_cores) + ".vhd", 'w') as f:
          with open(template_dir + '/tile_dpr.vhd', 'r') as ftemplate:
            for tline in ftemplate:
              if tline.find("-- <<reconfigurable_module_name>>") >= 0:
                f.write("entity tile_dpr_" + acc.name.upper() + "_PAR" + str(n_cores) + " is\n")
              elif tline.find("-- <<accelerator_parameters>>") >= 0:
                f.write("architecture rtl of tile_dpr_" + acc.name.upper() + "_PAR" + str(n_cores) + " is\n")
                f.write("attribute keep_hierarchy : string;\n")
                f.write("attribute keep_hierarchy of rtl : architecture is \"yes\";\n\n")
                f.write("  --Parameters of the reconfigurable module\n")
                f.write("  --(see file tools/socketgen/socketgen.py, function gen_tile_dpr to learn how to generate accelerators with different parallelism levels for dpr tiles)\n")
                f.write("  constant this_hls_conf       : hlscfg_t      := HLSCFG_" + acc.name.upper() + "_" + impl.name.upper() + ";\n")
                f.write("  constant this_n_cores        : integer       := " + str(n_cores) + ";\n")
                f.write("  constant this_log2_n_cores   : integer       := " + str(int(math.log2(n_cores))) + ";\n")
              elif tline.find("-- <<accelerator_instance>>") >= 0:
                f.write("    noc_" + acc.name + "_i: noc_" + acc.name + "\n")
                f.write("      generic map (\n")
                f.write("        hls_conf       => this_hls_conf,\n")
...
gen_tile_dpr(accelerator_list, axi_accelerator_list, template_dir, out_dir)
