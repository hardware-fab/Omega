...
static void print_usage() {
  ...
  printf("  --soft-reset        Send soft-reset to processor cores\n");
  printf("  --start-reset       Assert the hard reset to the whole SoC\n");
  printf("  --release-reset     Release the hard reset\n");
  printf("  --hard-reset        Assert and release the hard reset to the whole SoC\n");
...
static struct option long_options[] = {
        ...
        {"soft-reset",     no_argument,        0,  DO_SOFT_RESET      },
        {"start-reset",    no_argument,        0,  DO_START_RESET     },
        {"release-reset",  no_argument,        0,  DO_RELEASE_RESET   },
        {"hard-reset",     no_argument,        0,  DO_HARD_RESET      },
        {"listen",         no_argument,        0,  DO_LISTEN          },
        {0,                0,                  0,  0                  }
};
...
int main(int argc, char *argv[]) {
  ...
  case DO_SOFT_RESET :
    soft_reset(ESPLINK_BASE_ADDR);
    break;

  case DO_START_RESET :
    start_reset(HARD_RESET_BASE_ADDR);
    break;

  case DO_RELEASE_RESET :
    release_reset(HARD_RESET_BASE_ADDR);
    break;

  case DO_HARD_RESET :
    hard_reset(HARD_RESET_BASE_ADDR);
    break;
