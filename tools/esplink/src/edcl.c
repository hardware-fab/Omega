...
//GM change
//Implemento delle nuove funzioni per creare il pacchetto, perché ho la sensazione che quelle utilizzate qui non corrispondano alla direttiva del modulo hw ahbuart.vhd
...
//static void print_serial_conf(int serial_port)
//{
//  struct termios check_tty;
//  if(tcgetattr(serial_port, &check_tty) != 0)
//  {
//    printf("Error %i from tcgetattr: %s\n", errno, strerror(errno));
//  }
//  else
//  {
//    printf("PARENB = %d\n", check_tty.c_cflag & PARENB);
//    printf("CSTOPB = %d\n", check_tty.c_cflag & CSTOPB);
//    printf("CSIZE = %d\n", check_tty.c_cflag & CSIZE);
//    printf("CS8 = %d\n", check_tty.c_cflag & CS8);
//    printf("CRTSCTS = %d\n", check_tty.c_cflag & CRTSCTS);
//    printf("CREAD = %d\n", check_tty.c_cflag & CREAD);
//    printf("CLOCAL = %d\n", check_tty.c_cflag & CLOCAL);
//
//    printf("ICANON = %d\n", check_tty.c_lflag & ICANON);
//    printf("ECHO = %d\n", check_tty.c_lflag & ECHO);
//    printf("ECHOE = %d\n", check_tty.c_lflag & ECHOE);
//    printf("ECHONL = %d\n", check_tty.c_lflag & ECHONL);
//    printf("ISIG = %d\n", check_tty.c_lflag & ISIG);
//
//    printf("IXON = %d\n", check_tty.c_iflag & IXON);
//    printf("IXOFF = %d\n", check_tty.c_iflag & IXOFF);
//    printf("IXANY = %d\n", check_tty.c_iflag & IXANY);
//    printf("IGNBRK = %d\n", check_tty.c_iflag & IGNBRK);
//    printf("BRKINT = %d\n", check_tty.c_iflag & BRKINT);
//    printf("PARMRK = %d\n", check_tty.c_iflag & PARMRK);
//    printf("ISTRIP = %d\n", check_tty.c_iflag & ISTRIP);
//    printf("INLCR = %d\n", check_tty.c_iflag & INLCR);
//    printf("IGNCR = %d\n", check_tty.c_iflag & IGNCR);
//    printf("ICRNL = %d\n", check_tty.c_iflag & ICRNL);
//
//    printf("OPOST = %d\n", check_tty.c_oflag & OPOST);
//    printf("ONLCR = %d\n", check_tty.c_oflag & ONLCR);
//
//    printf("VTIME = %d\n", check_tty.c_cc[VTIME]);
//    printf("VMIN = %d\n", check_tty.c_cc[VMIN]);
//
//
//    printf("INPUT BAUDRATE is correct: %d\n", cfgetispeed(&check_tty) == B38400);
//    printf("OUTPUT BAUDRATE is correct: %d\n", cfgetospeed(&check_tty) == B38400);
//  }
//}
//GM change
...
//------------------------------------GM change: set UART connection----------------------------------------------
//(I've kept original names and parameters of the functions only for simplicity)
...
void start_reset(u32 addr)
{
  ...
  snd->write = 1;
  snd->address = addr;
  snd->data[0] = 0x01;   // 0x01 to assert the reset
  snd->length = 4;
  ...
  printf("Set hard reset of ESP SoC\n");
}

void release_reset(u32 addr)
{
  edcl_snd_t *snd = malloc(sizeof(edcl_snd_t));
  edcl_rcv_t *rcv = malloc(sizeof(edcl_rcv_t));
  ...
  snd->write = 1;
  snd->address = addr;
  snd->data[0] = 0x00;    // 0x00 to deassert the reset
  snd->length = 4;
  ...
  printf("Release hard reset of ESP SoC\n");
}

void hard_reset(u32 addr)
{
  // Reset must be sent twice: first to stop the execution and assert the reset,
  // and second to deactivate the reset
  start_reset(addr);
  release_reset(addr);
}
...
//GM change: funzione per leggere dati dalla seriale
