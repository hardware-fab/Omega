...
from clk_constraints_gen import *
from dpr_scripts_gen import *
...
class DPRFrame(Frame):

  def update_selected(self):
    selected_items = self.listbox.curselection()
    selected_values = [self.listbox.get(i) for i in selected_items]
    self.selected_label.config(text="N. of selected accelerators: " + str(len(selected_values)))
    self.soc.reconf_acc = selected_values

  def __init__(self, soc, top_frame, main_frame):
    self.soc = soc
    Frame.__init__(self, top_frame, width=50, borderwidth=2, relief=RIDGE)
    self.pack(side=LEFT, expand=NO, fill=Y)
    Label(self, text = "Dynamic Partial Reconfiguration", font="TkDefaultFont 11 bold").pack(side = TOP)

    dpr_config_frame = Frame(self)
    dpr_config_frame.pack(side=TOP)

    # List of options
    acc_list = tuple(soc.IPs.ACCELERATORS)

    # Create a Listbox
    self.listbox = Listbox(dpr_config_frame, selectmode=MULTIPLE)
    for acc_count in range(0, len(acc_list)):
      self.listbox.insert(END, acc_list[acc_count])
      if acc_list[acc_count] in self.soc.reconf_acc:
        self.listbox.selection_set(acc_count)
    self.listbox.pack()

    # Button to show selected values
    self.update_button = Button(dpr_config_frame, text="Update", command=self.update_selected)
    self.update_button.pack()

    # Inner frame
    inner_frame = Frame(dpr_config_frame)
    inner_frame.pack()

    # Label to display selected values
    self.selected_label = Label(inner_frame, text="N. of selected accelerators: " + str(len(self.soc.reconf_acc)))
    self.selected_label.grid(row=0, column=0, sticky="w")

    #Maximum number of cores
    Label(inner_frame, text="N. cores (max): ").grid(row=1, column=0, sticky="w")
    self.ncores_max = Entry(inner_frame, width=3, textvariable=self.soc.reconf_ncores)
    self.ncores_max.grid(row=1, column=1, sticky="w")

    #Default accelerator
    Label(inner_frame, text="Default acc: ").grid(row=2, column=0, sticky="w")
    Pmw.OptionMenu(inner_frame, menubutton_textvariable=soc.reconf_default, items=acc_list).grid(row=2, column=1, sticky="w")
...
class EspCreator(Frame):
  ...
  def __init__(self, master, _soc):
    ...
    #.:: creating the DPR frame
    self.dpr_frame = DPRFrame(self.soc, self.top_frame, self)
  ...
  def update_noc_config(self, *args):
    if soc.CPU_ARCH.get() == "ariane":
      self.soc.DMA_WIDTH = 64 #GM change: let's try and see if it works with a lower datawidth...
    else:
  ...
  def generate_socmap(self):
    ...
    #Collect data on planes number
    self.soc.noc.noc_planes = int(self.bottom_frame_noccfg.planes_entry.get())
    #Collect data on memory size
    self.soc.mem_size_main = int(self.bottom_frame_noccfg.main_mem_entry.get())
    self.soc.mem_size_secondary = int(self.bottom_frame_noccfg.secondary_mem_entry.get())
    ...
    #Function to create custom dfs constraints
    print_constraints(esp_config, soc)
    #Functions to create necessary files for DPR
    GenerateDprScripts(esp_config, soc)
    if esp_config.ndpr != 0:
      GenerateDefaultTileDpr(soc.reconf_default.get())
    GenerateFloorplanDpr(esp_config, soc)
