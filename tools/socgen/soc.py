...
class SoC_Config():
  ...
  def update_list_of_ips(self):
    self.list_of_ips = tuple(self.IPs.EMPTY) + tuple(self.IPs.PROCESSORS) + tuple(self.IPs.MISC) + tuple(self.IPs.MEM) + tuple(self.IPs.SLM) + tuple(self.IPs.DPR) + tuple(self.IPs.ACCELERATORS)
  ...
  def read_config(self, temporary):
    ...
    for y in range(0, self.noc.rows):
      for x in range(0, self.noc.cols):
        ...
        if len(tokens) > 1:
          ...
          if tokens[3] == "acc" or tokens[3] == "dpr" :
            tile.point.set(tokens[8])
            tile.has_l2.set(tokens[9])
            tile.vendor = tokens[10]
            #GM change: accelerator's parallelism
            if len(tokens) > 11:
              print("N CORES is present: n_cores = " + tokens[11])
              tile.n_cores.set(int(tokens[11]))
            else:
              print("N CORES is not present: n_cores = 1")
              tile.n_cores.set(1)
    ...
    line = fp.readline()
    item = line.split()
    noc_planes = int(item[2])
    self.noc.noc_planes = noc_planes
    ...
    for y in range(0, self.noc.rows):
      for x in range(0, self.noc.cols):
        ...
        if len(line) != 0:
          item = line.split()
        self.noc_clk_region.append(int(item[2]))

    #Read memory size
    line = fp.readline()
    if len(line) != 0:
      item = line.split()
      self.mem_size_main = int(item[2])
    else:
      self.mem_size_main = 2048

    line = fp.readline()
    if len(line) != 0:
      item = line.split()
      self.mem_size_secondary = int(item[2])
    else:
      self.mem_size_secondary = 512

    #Reconfigurable accelerators
    line = fp.readline() #CONFIG_HAS_DPR
    if line == "":
      return 0
    if line.split()[0] == "CONFIG_HAS_DPR":
      line = fp.readline() #DPR_ACC
    if len(line) != 0:
      item = line.split()
      for acc_count in range(2, len(item)):
        self.reconf_acc.append(item[acc_count])
    line = fp.readline() #DPR_NACC
    line = fp.readline() #DPR_NCORES
    if len(line) != 0:
      item = line.split()
      self.reconf_ncores.set(item[2])
    line = fp.readline() #DPR_DEFAULT
    if len(line) != 0:
      item = line.split()
      self.reconf_default.set(item[2])
    ...
    for y in range(0, self.noc.rows):
      for x in range(0, self.noc.cols):
        ...
        if self.IPs.PROCESSORS.count(selection):
          ...
          is_slm = True
          fp.write("slm")
        elif self.IPs.DPR.count(selection):
          is_dpr = True
          has_dpr = 1
          fp.write("dpr")
        ...
        # Accelerator tile configuration
        if is_accelerator or is_dpr:
          fp.write(" " + str(tile.point.get()))
          fp.write(" " + str(tile.has_l2.get()))
          fp.write(" " + str(tile.vendor))
          #GM change: accelerator's parallelism
          if is_accelerator:
            fp.write(" " + str(tile.n_cores.get()))
        fp.write("\n")
        i += 1
    if has_dvfs:
    ...
    for y in range(self.noc.rows):
      for x in range(self.noc.cols):
        ...
        if self.IPs.ACCELERATORS.count(selection) == 0:
          ...
          fp.write("\n")
    #Write noc planes number
    fp.write("CONFIG_NOC_PLANES = " + str(self.noc.noc_planes) + "\n")
    #NoC domains
    for y in range(0, self.noc.rows):
      for x in range(0, self.noc.cols):
        if x+y*self.noc.cols >= len(self.noc_clk_region):
          fp.write("NOC_" + str(y) + "_" + str(x) + " = 0\n")
        else:
          fp.write("NOC_" + str(y) + "_" + str(x) + " = " + str(self.noc_clk_region[x+y*self.noc.cols]) + "\n")
    #Write memory size
    fp.write("MEM_SIZE_MAIN = " + str(self.mem_size_main) + "\n")
    fp.write("MEM_SIZE_SECONDARY = " + str(self.mem_size_secondary) + "\n")
    #Write reconfigurable accelerators
    if (len(self.reconf_acc) > 0):
      fp.write("CONFIG_HAS_DPR = ")
      if has_dpr:
        fp.write("1\n")
      else:
        fp.write("0\n")
      fp.write("DPR_ACC = ")
      for acc in self.reconf_acc:
        fp.write(acc + " ")
      fp.write("\n")
      fp.write("DPR_NACC = " + str(len(self.reconf_acc)) + "\n")
      fp.write("DPR_NCORES = " + str(self.reconf_ncores.get()) + "\n")
      fp.write("DPR_DEFAULT = " + str(self.reconf_default.get()) + "\n")
  ...
  def __init__(self, DMA_WIDTH, TECH, LINUX_MAC, LEON3_STACK, FPGA_BOARD, EMU_TECH, EMU_FREQ, temporary):
    ...
    if self.FPGA_BOARD == "profpga-xc7v2000t":
      self.nmem_max = 4  #GM change: putting 4 instead of 2
    elif self.FPGA_BOARD == "xilinx-u55c-xcu55c":
      self.nmem_max = 4
    elif self.FPGA_BOARD.find("xilinx") != -1:
      self.nmem_max = 1
    else:
      self.nmem_max = 4
    ...
    #Noc domains variable
    self.noc_clk_region = []

    #Memory size
    self.mem_size_main = 2048         #Size in KB of the first memory tile
    self.mem_size_secondary = 512     #Size in KB of each memory tile after the first one

    #Reconfigurable accelerators
    self.reconf_acc = []
    self.reconf_ncores = IntVar()
    self.reconf_default = StringVar()
