...
# GM change: my memories are way smaller than 2GB
# Note that this is the sum of all modules
#DDR_SIZE = 0x400
DDR_SIZE = 0x004
...
class acc_info:
  ...
  idx = -1
  irq = 5
  #GM change: accelerator parallelism
  n_cores = 1
...
class tile_info:
  ...
  has_pll = 0
  has_clkbuf = 0
  #GM change: among tiles info I add the clock domain to which the noc belongs
  noc_clk_region = 0;
  #GM change: accelerator's parallelism
  n_cores = 1
...
class soc_config:
  ...
  def __init__(self, soc):
    ...
    self.ndpr = soc.noc.get_dpr_num(soc)
    ...
    #GM change: memory size
    self.mem_size_main = soc.mem_size_main
    self.mem_size_secondary = soc.mem_size_secondary
    ...
    for x in range(soc.noc.rows):
      for y in range(soc.noc.cols):
        ...
        #GM change: initialize the noc clock region
        if len(soc.noc_clk_region) > t:
          self.tiles[t].noc_clk_region = soc.noc_clk_region[t]
        else:
          self.tiles[t].noc_clk_region = 0
        ...
        if selection == "dpr":
          self.tiles[t].type = "dpr"
          dpr = acc_info()  #DPR tiles are, in fact, accelerators
          dpr.uppercase_name = "DPR"
          dpr.lowercase_name = "dpr"
          dpr.id = acc_id   #Same ID counting as for normal accelerators
          dpr.irq = acc_irq
          dpr.idx = SLD_APB_PINDEX + acc_id
          dpr.vendor = soc.noc.topology[x][y].vendor
          #GM change: accelerator's parallelism
          dpr.n_cores = 1
          self.tiles[t].acc = dpr
          self.accelerators.append(dpr)
          acc_id = acc_id + 1
          if self.cpu_arch == "ariane" or self.cpu_arch == "ibex":
            acc_irq = acc_irq + 1
        ...
        if soc.IPs.ACCELERATORS.count(selection):
          ...
          #GM change: accelerator's parallelism
          acc.n_cores = soc.noc.topology[x][y].n_cores.get()
          if acc.vendor != "sld":
              self.nthirdparty += 1
...
def print_global_constants(fp, soc):
  ...
  #GM change: I really need the number of NoC as a global parameter
  fp.write("  constant num_noc_planes : integer := " + str(soc.noc.noc_planes) + ";\n\n")
...
def print_constants(fp, soc, esp_config):
  ...
  fp.write("  constant CLOCK_DOMAINS_NUM : integer := " + str(esp_config.ndomain) + ";  --GM change: I need this info as a global variable due to package dependence issues\n\n");
  #
  ...
  #GM change: memory size
  fp.write("  ------ GM change: memory size\n")
  fp.write("  constant CFG_MEM_SIZE_MAIN : integer := " + str(soc.mem_size_main) + ";\n")
  fp.write("  constant CFG_MEM_SIZE_ADD  : integer := " + str(soc.mem_size_secondary) + ";\n\n")
...
def print_mapping(fp, soc, esp_config):
  ...
  for i in range(0, esp_config.nmem):
    #fp.write("    " + str(i) + " => 16#" + format(offset, '03X') + "#,\n")
    fp.write("    " + str(i) + " => 16#" + format(int(offset), '03X') + "#,\n")
    #offset = offset + size
    #Two different sizes for main and secondary memories (expressed in KB)
    #(Note: if secondary memory is smaller than 1MB, we make the SoC believe that its size is 1MB to avoid issues)
    if i == 0:
      offset = offset + soc.mem_size_main/1024
    else:
      if soc.mem_size_secondary >= 1024:
        offset = offset + soc.mem_size_secondary/1024
      else:
        offset = offset + 1
  fp.write("    others => 0);\n")
  ...
  for i in range(0, esp_config.nmem):
    #fp.write("    " + str(i) + " => 16#" + format(mask, '03X') + "#,\n")
    #GM change: a specific mask for the memory
    #(Note: if secondary memory is smaller than 1MB, we make the SoC believe that its size is 1MB to avoid issues)
    local_mask = 0
    if i == 0:
      local_mask = 0xfff & ~(int(soc.mem_size_main/1024) - 1)
    else:
      if soc.mem_size_secondary >= 1024:
        local_mask = 0xfff & ~(int(soc.mem_size_secondary/1024) - 1)
      else:
        local_mask = 0xfff

    fp.write("    " + str(i) + " => 16#" + format(local_mask, '03X') + "#,\n")
  fp.write("    others => 0);\n")
  ...
  for i in range(esp_config.nacc):
    ...
    #GM change: adding the number of cores for accelerator parallelism
    fp.write("  constant " + acc.lowercase_name + "_" + str(acc.id) + "_ncores : integer range 1 to 32 := " + str(acc.n_cores) + ";\n")
    fp.write("  constant " + acc.lowercase_name + "_" + str(acc.id) + "_log2_ncores : integer range 1 to 5 := " + str(math.ceil(math.log2(acc.n_cores))) + ";\n")
    fp.write("  constant " + acc.lowercase_name + "_" + str(acc.id) + "_pindex : integer range 0 to NAPBSLV - 1 := " + str(acc.idx) + ";\n")
    #GM change: since the interrupt max number is limiting the number of accelerators I can have, I remove the interrupts on accelerators altogether
    #fp.write("  constant " + acc.lowercase_name + "_" + str(acc.id) + "_pirq : integer range 0 to NAHBIRQ - 1 := " + str(acc.irq) + ";\n")
    fp.write("  constant " + acc.lowercase_name + "_" + str(acc.id) + "_pirq : integer range 0 to NAHBIRQ - 1 := " + str(0) + ";\n")
    if acc.vendor == "sld":
    ...
    fp.write("  0 => ahb_device_reg (VENDOR_SLD, SLD_" + acc.uppercase_name + ", 0, 0, " + str(0) + "),\n")
    fp.write("  1 => apb_iobar(16#" + address_str + "#, 16#" + msk_str  + "#),\n")
    if address_ext == 0:
      fp.write("  2 => (others => '0'));\n\n")
    else:
  ...
  #
  fp.write("  -- GM change: Get accelerator parallelism level from tile ID\n")
  fp.write("  constant tile_n_cores : attribute_vector(0 to CFG_TILES_NUM - 1) := (\n")
  for i in range(0, esp_config.ntiles):
    t = esp_config.tiles[i]
    if t.acc.id != -1:
      fp.write("    " + str(i) + " => " + str(t.acc.n_cores) + ",\n")
  fp.write("    others => 0);\n\n")

  #
  fp.write("  -- GM change: Get accelerator parallelism level from accelerator ID\n")
  fp.write("  constant acc_n_cores : attribute_vector(0 to CFG_TILES_NUM - 1) := (\n")
  for i in range(0, esp_config.ntiles):
    t = esp_config.tiles[i]
    if t.acc.id != -1:
      fp.write("    " + str(t.acc.id) + " => " + str(t.acc.n_cores) + ",\n")
  fp.write("    others => 0);\n\n")

  #
  fp.write("  -- GM change: Get log2 of accelerator parallelism level from tile ID\n")
  fp.write("  constant tile_log2_n_cores : attribute_vector(0 to CFG_TILES_NUM - 1) := (\n")
  for i in range(0, esp_config.ntiles):
    t = esp_config.tiles[i]
    if t.acc.id != -1:
      fp.write("    " + str(i) + " => " + str(math.ceil(math.log2(t.acc.n_cores))) + ",\n")
  fp.write("    others => 0);\n\n")

  #
  fp.write("  -- GM change: Get log2 of accelerator parallelism level from accelerator ID\n")
  fp.write("  constant acc_log2_n_cores : attribute_vector(0 to CFG_TILES_NUM - 1) := (\n")
  for i in range(0, esp_config.ntiles):
    t = esp_config.tiles[i]
    if t.acc.id != -1:
      fp.write("    " + str(t.acc.id) + " => " + str(math.ceil(math.log2(t.acc.n_cores))) + ",\n")
  fp.write("    others => 0);\n\n")
  ...
  for i in range(0, esp_config.ntiles):
    ...
    if esp_config.tiles[i].type == "mem":
      type = 4
    if esp_config.tiles[i].type == "dpr":
      type = 5
  ...
  for i in range(0, esp_config.ntiles):
    ...
    if tile.type == "acc" or tile.type == "dpr":
  ...
  for i in range(0, esp_config.ntiles):
    if esp_config.tiles[i].type == "acc" or esp_config.tiles[i].type == "dpr":
  ...
  #GM change: parameter for the noc domain
  fp.write("  -- GM change: get noc clock domain from tile ID\n")
  fp.write("  constant noc_domain : attribute_vector(0 to CFG_TILES_NUM - 1) := (\n")
  if esp_config.has_dvfs:
    for i in range(0, esp_config.ntiles):
      fp.write("    " + str(i) + " => " + str(esp_config.tiles[i].noc_clk_region) + ",\n")
  fp.write("    others => 0);\n\n")

  # GM change: this is an extra parameter for the frequency of a certain clock domain (not needed anymore)
  fp.write("  -- GM change: get frequency from domain ID\n")
  fp.write("  constant domain_freq : attribute_vector(0 to domains_num - 1) := (\n")
  #for i in range(0, 5):
  #  fp.write("    " + str(i) + " => " + str(esp_config.domains[i].freq) + ",\n")
  fp.write("    others => 0);\n\n")
  ...
  for i in range(0, esp_config.ntiles):
    #GM change: the domain master of a memory tile is the tile itself (that is, the icclk)
    if esp_config.tiles[i].mem_id != -1 :
      fp.write("    " + str(i) + " => " + str(i) + ",\n")
    else:
      fp.write("    " + str(i) + " => " + str(pll_tile[esp_config.tiles[i].clk_region]) + ",\n")
  fp.write("    others => 0);\n\n")

  #GM change: get the domain master ID of a noc
  fp.write("  -- GM change: get tile ID of the DVFS domain master from the noc clock region\n")
  fp.write("  constant noc_domain_master : attribute_vector(0 to CFG_TILES_NUM - 1) := (\n")
  for i in range(0, esp_config.ntiles):
    #GM change: the master of region zero is the memory tile
    if esp_config.tiles[i].noc_clk_region == 0:
      for k in  range(0, esp_config.ntiles):
        t = esp_config.tiles[k]
        if t.mem_id == 0:
          fp.write("    " + str(i) + " => " + str(k) + ",\n")
    else:
      fp.write("    " + str(i) + " => " + str(pll_tile[esp_config.tiles[i].noc_clk_region]) + ",\n")
  fp.write("    others => 0);\n\n")
...
def print_socmap_functions(fp):
  fp.write("  -- This function finds the number of tiles for a specific clock domain\n")
  fp.write("  function get_tiles_number_in_clock_domain (\n")
  fp.write("  domain_id : integer)\n")
  fp.write("  return integer;\n")
  fp.write("\n")
  fp.write("  -- This function finds the position of a tile among all the tiles belonging to the same clock domain\n")
  fp.write("  function get_tile_input_clock (\n")
  fp.write("  tile_id : integer)\n")
  fp.write("  return integer;\n")
  fp.write("\n")
  fp.write("end;\n")
  fp.write("\n")
  fp.write("package body socmap is\n")
  fp.write("\n")
  fp.write("  -- This function finds the number of tiles for a specific clock domain\n")
  fp.write("  function get_tiles_number_in_clock_domain (\n")
  fp.write("  domain_id : integer)\n")
  fp.write("  return integer is\n")
  fp.write("  variable counter : integer := 0;\n")
  fp.write("  begin\n")
  fp.write("    counter := 0;\n")
  fp.write("    for i in 0 to CFG_TILES_NUM-1 loop\n")
  fp.write("      if tile_domain(i) = domain_id then\n")
  fp.write("        counter := counter + 1;\n")
  fp.write("      end if;\n")
  fp.write("    end loop;\n")
  fp.write("    return counter;\n")
  fp.write("  end;\n")
  fp.write("  \n")
  fp.write("  -- This function finds the position of a tile among all the tiles belonging to the same clock domain\n")
  fp.write("  function get_tile_input_clock (\n")
  ...
  fp.write("  begin\n")
  fp.write("    counter := 0;\n")
  fp.write("    for i in 0 to tile_id-1 loop\n")
  fp.write("      if tile_domain(i) = tile_domain(tile_id) then\n")
  fp.write("        counter := counter + 1;\n")
  fp.write("      end if;\n")
  fp.write("    end loop;\n")
  fp.write("    return counter;\n")
...
def print_esplink_header(fp, esp_config, soc):
  ...
  fp.write("#define HARD_RESET_BASE_ADDR 0x" + format(AHB2APB_HADDR[esp_config.cpu_arch], '03X') + "003FC\n")
...
def print_soc_defines(fp, esp_config, soc):
  ...
  fp.write("//GM change: memory size\n");
  fp.write("#define MEM_SIZE_MAIN 0x" + format(int(soc.mem_size_main*1024), '08X') + "\n");
  #(Note: if secondary memory is smaller than 1MB, we make the SoC believe that its size is 1MB to avoid issues)
  if soc.mem_size_secondary >= 1024:
    fp.write("#define MEM_SIZE_ADD  0x" + format(int(soc.mem_size_secondary*1024), '08X') + "\n\n");
  else:
    fp.write("#define MEM_SIZE_ADD  0x" + format(int(1024*1024), '08X') + "\n\n");
...
def print_soc_locations(fp, esp_config, soc):
  ...
  if esp_config.nacc > 0:
   ...
   #fp.write("\n// GM change: accelerator parallelism\n")
   #fp.write("unsigned int acc_par_lvls[" + str(esp_config.nacc) + "] = {")
   #for i in range(0, esp_config.ntiles):
   #  t = esp_config.tiles[i]
   #  if t.type == "acc":
   #    fp.write(str(t.acc.n_cores))
   #    if not t.acc.id == esp_config.nacc - 1:
   #      fp.write(", ")
   #fp.write("};\n")
