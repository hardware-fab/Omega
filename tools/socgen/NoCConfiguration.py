...
class Tile():
  ...
  def update_tile(self, soc):
    ...
    elif soc.IPs.DPR.count(selection):
       self.label.config(bg="#10e0e0")
       self.point.set("NONE")
       self.vendor = "sld"
    ...
    self.n_cores_selection.config(to=32)


    try:
      if soc.IPs.PROCESSORS.count(selection) or soc.IPs.ACCELERATORS.count(selection) or soc.IPs.DPR.count(selection):
         self.clk_reg_selection.config(state='readonly')
         #GM change: accelerator's parallelism
         if soc.IPs.ACCELERATORS.count(selection):
           self.n_cores_selection.config(state='readonly')
         else:
           self.n_cores_selection.config(state=DISABLED)
  ...
  def power_window(self, event, soc, nocframe):
    selection = self.ip_type.get()
    if soc.IPs.ACCELERATORS.count(selection) == 0 or soc.IPs.DPR.count(selection) == 0 or self.clk_region.get() == 0:
      return
    try:
    ...
    for y in range(_R):
      ...
      for x in range(_C):
        ...
        if x < self.cols and y < self.rows:
          ...
          #GM change: accelerator parallelism
          new_topology[y][x].n_cores.set(self.topology[y][x].n_cores.get())
  ...
  def get_clkbuf_num(self, soc):
    ...
    for y in range(0, self.rows):
      for x in range(0, self.cols):
         ...
         if (soc.IPs.ACCELERATORS.count(selection) or soc.IPs.DPR.count(selection)) and tile.has_clkbuf.get() == 1:
            tot_clkbuf += 1
    return tot_clkbuf
  ...
  def get_acc_num(self, soc):
    ...
    for y in range(0, self.rows):
      for x in range(0, self.cols):
         tile = self.topology[y][x]
         selection = tile.ip_type.get()
         if soc.IPs.ACCELERATORS.count(selection) or soc.IPs.DPR.count(selection):
            tot_acc += 1
    return tot_acc

  def get_dpr_num(self, soc):
    tot_dpr = 0
    for y in range(0, self.rows):
      for x in range(0, self.cols):
         tile = self.topology[y][x]
         selection = tile.ip_type.get()
         if soc.IPs.DPR.count(selection):
            tot_dpr += 1
    return tot_dpr
...
class NoCFrame(Pmw.ScrolledFrame):
  ...
  def changed(self,*args):
    if isInt(self.ROWS.get()) == False or isInt(self.COLS.get()) == False:
       return
    #print("TOPOLOGY SIZE: X=" + str(len(self.noc.topology[0])) + ", Y=" + str(len(self.noc.topology)))
  ...
  def create_tile(self, frame, tile):
    ...
    #GM note: this is the baseline width for all the elements in the tile square
    width = 0
    ...
    tile.ip_list = Pmw.OptionMenu(select_frame, menubutton_font="TkDefaultFont 8",
                   ...
                   menubutton_width = width+2, #GM note: size of the button on top of the square
                   items=list_items
                  )
    tile.ip_list.pack(side=LEFT)
    tile.point_label = Label(select_frame, text="Impl.: ", width=5) #GM note: size of the Impl. word in accelerator tiles
    tile.point_select = Pmw.OptionMenu(select_frame, menubutton_font="TkDefaultFont 8",
                   menubutton_textvariable=tile.point,
                   menubutton_width = 10, #GM note: size of the button to select dma implementation
                   items=[]
                  )

    tile.label = Label(display_frame, text=tile.ip_type.get())
    tile.label.config(height=4,bg='white', width=width+25) #GM note: size of the coloured square
    tile.label.pack()
    ...
    #GM change: field to insert the parallelism level
    Label(config_frame, text="N Cores: ", justify=LEFT, anchor="w").grid(sticky = E, row=3, column=4)
    tile.n_cores_selection = Spinbox(config_frame, state='readonly', from_=1, to=32, wrap=True, textvariable=tile.n_cores, width=3, justify=RIGHT);
    tile.n_cores_selection.grid(sticky = W, row=3, column=5)
    try:
  ...
  def __init__(self, soc, bottom_frame):
    ...
    #GM change: adding an entry for the number of NoC planes
    self.planes_frame = Frame(self.noc_config_frame)
    self.planes_frame.pack(side=TOP, fill=BOTH)
    Label(self.planes_frame, anchor=W, width=10, text=" NoC planes: ").pack(side=LEFT)
    self.planes_entry = Entry(self.planes_frame, width=3)
    self.planes_entry.pack(side=LEFT)
    self.planes_entry.delete(0, END)
    self.planes_entry.insert(0, str(soc.noc.noc_planes))

    #GM change: adding an entry for the size of the main memory
    self.main_mem_frame = Frame(self.noc_config_frame)
    self.main_mem_frame.pack(side=TOP, fill=BOTH)
    Label(self.main_mem_frame, anchor=W, width=25, text=" Main memory size (KB): ").pack(side=LEFT)
    self.main_mem_entry = Entry(self.main_mem_frame, width=5)
    self.main_mem_entry.pack(side=LEFT)
    self.main_mem_entry.delete(0, END)
    self.main_mem_entry.insert(0, str(soc.mem_size_main))

    #GM change: adding an entry for the size of each additional memory
    self.secondary_mem_frame = Frame(self.noc_config_frame)
    self.secondary_mem_frame.pack(side=TOP, fill=BOTH)
    Label(self.secondary_mem_frame, anchor=W, width=25, text=" Additional memories size (KB): ").pack(side=LEFT)
    self.secondary_mem_entry = Entry(self.secondary_mem_frame, width=5)
    self.secondary_mem_entry.pack(side=LEFT)
    self.secondary_mem_entry.delete(0, END)
    self.secondary_mem_entry.insert(0, str(soc.mem_size_secondary))
  ...
  def update_msg(self):
    ...
    #GM change: verify that the parallelism level of each accelerator is a power of two
    n_cores_string = ""
    n_cores_ok = True
    for y in range(0, self.noc.rows):
      for x in range(0, self.noc.cols):
        tile = self.noc.topology[y][x]
        selection = tile.ip_type.get()
        if self.soc.IPs.ACCELERATORS.count(selection):
          if tile.n_cores.get() != 1 and tile.n_cores.get() != 2 and tile.n_cores.get() != 4 and tile.n_cores.get() != 8 and tile.n_cores.get() != 16 and tile.n_cores.get() != 32:
            n_cores_ok = False
            n_cores_string += "Accelerator in tile (" + str(y) + "," + str(x) + ") has a number of cores which is not a power of 2\n"
    ...
    if (tot_cpu > 0) and \
      ...
      #GM change: removing the limit on three memories. Does it work?
      #if (tot_mem == 3):
      ...
      string += pll_string
      string += n_cores_string #GM change
      if (clk_region_skip > 0):
        string += "Clock-region IDs must be consecutive; skipping region " + str(clk_region_skip) +" instead\n"
    # Update message box
  ...
  def create_noc(self):
    ...
    for y in range(0, int(self.ROWS.get())):
      for x in range(0, int(self.COLS.get())):
        ...
        #GM change: accelerator's parallelism
        tile.n_cores.trace('w', self.changed)
    self.changed()
