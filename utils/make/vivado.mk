...
#Adding constraints for DFS and DPR
XDC  += $(DESIGN_PATH)/socgen/esp/dfs_constraints.xdc
XDC  += $(DESIGN_PATH)/socgen/esp/dpr_floorplan.xdc
endif
endif
...
ifneq ($(IP_XCI_SRCS),)
	...
	@if test -r $(ESP_ROOT)/constraints/$(BOARD)/hbm_interface.xci; then \
		echo $(SPACES)"INFO including HBM IP"; \
		mkdir -p vivado/hbm_interface; \
		cp $(ESP_ROOT)/constraints/$(BOARD)/hbm_interface.xci ./vivado/hbm_interface; \
		echo "import_ip -files ./hbm_interface/hbm_interface.xci" >> $@; \
		echo "generate_target  all [get_ips hbm_interface] -force " >> $@; \
	fi;
	@if test -r $(ESP_ROOT)/constraints/$(BOARD)/axi_dwidth_converter_ip.xci; then \
		echo $(SPACES)"INFO including axi_dwidth_converter IP"; \
		mkdir -p vivado/axi_dwidth_converter_ip; \
		cp $(ESP_ROOT)/constraints/$(BOARD)/axi_dwidth_converter_ip.xci ./vivado/axi_dwidth_converter_ip; \
		echo "import_ip -files ./axi_dwidth_converter_ip/axi_dwidth_converter_ip.xci" >> $@; \
		echo "generate_target  all [get_ips axi_dwidth_converter_ip] -force " >> $@; \
	fi;
...
vivado/syn.tcl: vivado
	...
	@echo "launch_runs impl_1 -jobs 12 -to_step write_bitstream" >> $@
	@echo "wait_on_run impl_1" >> $@
	@echo "open_run impl_1" >> $@
	@echo "report_utilization -file $(DESIGN_PATH)/vivado/esp-$(BOARD).runs/impl_1/hierarchical_utilization.rpt -hierarchical" >> $@
	@if [ "$(CONFIG_HAS_DPR)" = "1" ]; then \
		log2_value=0; \
		val=$(DPR_NCORES); \
		while [ $$val -gt 1 ]; do \
		val=$$((val >> 1)); \
		log2_value=$$((log2_value + 1)); \
		done; \
		n_impl=$$(( 1 + (log2_value + 1) * $(DPR_NACC))); \
		for i in $$(seq 1 $$n_impl); do \
		echo "launch_runs child_$${i}_impl_1 -jobs 12 -to_step write_bitstream" >> $@; \
		echo "wait_on_run child_$${i}_impl_1" >> $@; \
		echo "open_run child_$${i}_impl_1" >> $@; \
		echo "report_utilization -file $(DESIGN_PATH)/vivado/esp-$(BOARD).runs/child_$${i}_impl_1/hierarchical_utilization.rpt -hierarchical" >> $@; \
		done \
	else \
		echo "No reconfigurable tiles"; \
	fi
...
vivado/program.tcl: vivado
	...
	@echo "#Temporary modification: digilent cable does not work with profpga" >> $@
	@echo "if [string match -nocase "*Digilent*" \$$cable] {" >> $@
	@echo "continue" >> $@
	@echo "}" >> $@
	@echo "#GM change: I must use the other board" >> $@
	@echo "if [string match -nocase "*XFL13CWPGUVRA*" \$$cable] {" >> $@
	@echo "continue" >> $@
	@echo "}" >> $@
	@echo "    open_hw_target \$$cable" >> $@
	@echo "    set dev [get_hw_devices]" >> $@
	@echo "    if [string match -nocase \"\$$part*\" \$$dev] {" >> $@
	@echo "	puts \"Programming \$$part with \$$bit...\"" >> $@
...
vivado-syn: vivado-setup
	...
	@cd vivado; \
	vivado $(VIVADO_BATCH_OPT) -source $(DESIGN_PATH)/socgen/esp/dpr_flow.tcl; \
...
vivado-prog-fpga-partial: vivado/program.tcl
	@echo "Loading bitstream $(BIT)..."
	@cd vivado; \
	if test -r $(BIT); then \
		echo "Bash command: vivado $(VIVADO_BATCH_OPT) -source program.tcl -tclargs $(FPGA_HOST) $(XIL_HW_SERVER_PORT) $(PART) $(BIT)"; \
		vivado $(VIVADO_BATCH_OPT) -source program.tcl -tclargs $(FPGA_HOST) $(XIL_HW_SERVER_PORT) $(PART) $(BIT); \
	else \
		echo $(SPACES)"ERROR: bistream not found; please run target vivado-syn first"; \
	fi; \
